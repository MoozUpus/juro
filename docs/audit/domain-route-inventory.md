...
Repository baseline: `origin/main` at merge commit `8b650f566ec2f5a9511565a6fdb62eca8a86a7b6`
...
| [status.juro.uz](https://status.juro.uz/) | production | Public-safe operational status; anonymous | Cloudflare Worker domain; `STATUS_HOSTNAME` | Root and `/api/status` return `200`; v179 snapshot reports 7/8 components operational and AI degraded | `noindex`; public-safe telemetry | OpenAI probe remains `PROVIDER_UNAVAILABLE`; retain the truthful degraded label and no-sensitive-data boundary | `PARTIAL` |
