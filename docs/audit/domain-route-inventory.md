...
| [app.juro.uz](https://app.juro.uz/) | production | Client and business platform; auth required; RU/UZ | Cloudflare Worker v189 and `apps/platform/wrangler.jsonc` | One redirect to localized login, final `200`; public lawyer API returns only approved profiles | HTML and header both `noindex`; private data | Fresh status and directory contracts are operational; authenticated Client/Business flow remains incomplete | `PARTIAL` |
| [lawyer.juro.uz](https://lawyer.juro.uz/) | production | Lawyer entry and workspace; public entry plus protected routes; RU/UZ | Cloudflare Worker v189 and platform host router | `/ru/individual/dashboard` redirects to `/ru/auth/login?returnTo=%2Fru%2Flawyer%2Fdashboard`; no `Not Found`, and Chrome logged no console errors | `noindex`; private/professional data | Production keeps the dedicated lawyer persona; authenticated Lawyer Chrome flow remains open | `PARTIAL` |
...
| [status.juro.uz](https://status.juro.uz/) | production | Public-safe operational status; anonymous | Cloudflare Worker domain; `STATUS_HOSTNAME` | Root and `/api/status` return `200`; `2026-09-01T15:33:22.376Z` snapshot reports 8/8 components operational and no active incidents | `noindex`; public-safe telemetry | OpenAI and Anthropic probes recovered after the earlier redacted `PROVIDER_UNAVAILABLE` state; treat this as point-in-time health, not sustained availability | `PARTIAL` |
...
5. **Open evidence gate:** authenticated Client, Business, Lawyer, Pending Lawyer, and Staff/Admin Chrome runs are still required before these families can be marked `VERIFIED`. No session was fabricated. Anonymous Chrome evidence on Worker v189 preserves the lawyer persona boundary and the safe app login redirect.
6. **P1 — sustained production AI evidence:** the public status API recovered to 8/8 at `2026-09-01T15:33:22.376Z`, with fresh successful OpenAI and Anthropic probes. Sustained probe success plus authenticated AI and document-analysis journeys remain required before the provider gate is considered complete.
