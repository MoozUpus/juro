...
Repository baseline: `origin/main` at merge commit `cd3ee161bb4a54c7bdc71b89c39a402f3ad35c4d`; live v115 Workers and public Sites v96 were verified independently after deployment
...
- Sites project `appgprj_6a5e1b9547e88191bf759bbeae44d315`, live version 96;
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| [juro.uz](https://juro.uz/) | production | Public website; anonymous; RU/UZ/EN | Cloudflare apex A records; Sites hosting metadata | `200`; `/`, `/ru`, `/uz`, `/en` all `200`; 78/78 sitemap URLs and 120/120 discoverable apex links `200` after v96 with self-canonical metadata | `index, follow`; public | v96 preserves the custom-domain indexing contract; saved v95 is the immediate rollback | `VERIFIED` |
...
| [juro-legaltech.muzaffarbekmurodoff.chatgpt.site](https://juro-legaltech.muzaffarbekmurodoff.chatgpt.site/) | production provider surface | Sites service hostname; anonymous; RU/UZ/EN content clone | Sites deployment result | `200`; canonical points to `https://juro.uz/ru` | v96 returns `X-Robots-Tag: noindex, nofollow, noarchive` on the localized route, `robots.txt`, and `sitemap.xml` | Duplicate-host indexing boundary remains enforced while `juro.uz` remains indexable | `VERIFIED` |
| [app.juro.uz](https://app.juro.uz/) | production | Client and business platform; auth required; RU/UZ | Platform Worker v115 and `apps/platform/wrangler.jsonc` | One redirect to localized login, final `200`; authenticated individual read-only routes were sampled, while Business remains open | HTML and header both `noindex`; private data | Fresh status and directory contracts are operational; authenticated Business and state-changing flows remain incomplete | `PARTIAL` |
| [lawyer.juro.uz](https://lawyer.juro.uz/) | production | Lawyer entry and workspace; public entry plus protected routes; RU/UZ | Platform Worker v115 and platform host router | Mobile login preserves the lawyer persona; heading/control overlap is 0 px², Turnstile is visible, and submit remains disabled before verification | `noindex`; private/professional data | Production keeps the dedicated lawyer persona; authenticated Lawyer Chrome flow remains open | `PARTIAL` |
...
| [status.juro.uz](https://status.juro.uz/) | production | Public-safe operational status; anonymous | Cloudflare Worker domain; `STATUS_HOSTNAME` | Root and `/api/status` return `200`; snapshot generated `2026-09-01T22:10:42.083Z` reports operational, 8/8 components, and no active incidents | `noindex`; public-safe telemetry | Point-in-time evidence does not prove sustained provider health | `VERIFIED` for the checked snapshot |
...
1. **Resolved — provider-host indexing:** Sites v96 is live. The provider hostname returns `X-Robots-Tag: noindex, nofollow, noarchive`, while `juro.uz` retains `index, follow`; 78/78 sitemap URLs and 120/120 discoverable apex links return `200`.
...
5. **Open evidence gate:** authenticated Business, Lawyer, Pending Lawyer, and Staff/Admin Chrome runs are still required before these families can be marked `VERIFIED`; authenticated individual read-only coverage is partial and state-changing flows remain open. No session was fabricated. Production v115 preserves the lawyer persona boundary and public-approved directory contract.
6. **Monitor — production AI providers:** the checked v115-era snapshot reports fresh successful OpenAI and Anthropic probes and overall operational status. This is point-in-time evidence only; continue normal monitoring before making a sustained-health claim.
