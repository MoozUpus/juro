...
Repository baseline: `origin/main` at merge commit `3575ed3aff26904ac2d166c0c2be38f1b94b9755`; live v116 Workers and public Sites v97 were verified independently after deployment
...
| [app.juro.uz](https://app.juro.uz/) | production | Client and business platform; auth required; RU/UZ | Platform Worker v202 and `apps/platform/wrangler.jsonc` | One redirect to localized login, final `200`; authenticated Individual shell was sampled at four viewports; Business remains open | HTML and header both `noindex`; private data | Individual read-only shell and persona enforcement are verified; authenticated Business and state-changing flows remain incomplete | `PARTIAL` |
| [lawyer.juro.uz](https://lawyer.juro.uz/) | production | Lawyer entry and workspace; public entry plus protected routes; RU/UZ | Platform Worker v202 and platform host router | Login preserves the lawyer persona at four viewports; heading/control overlap is 0 px², Turnstile is visible, and submit remains protected before verification | `noindex`; private/professional data | Production keeps the dedicated lawyer persona; authenticated Lawyer Chrome flow remains open | `PARTIAL` |
...
| [status.juro.uz](https://status.juro.uz/) | production | Public-safe operational status; anonymous | Cloudflare Worker domain; `STATUS_HOSTNAME` | Root and `/api/status` return `200`; snapshot generated `2026-09-01T23:18:39.567Z` reports operational, 8/8 components, and no active incidents | `noindex`; public-safe telemetry | Point-in-time evidence does not prove sustained provider health | `VERIFIED` for the checked snapshot |
...
5. **Open evidence gate:** authenticated Business, Lawyer, Pending Lawyer, and Staff/Admin Chrome runs are still required before these families can be marked `VERIFIED`; authenticated Individual shell coverage is read-only and state-changing flows remain open. No session was fabricated. Production v116 preserves the lawyer persona boundary and public-approved directory contract.
6. **Monitor — production AI providers:** the checked v116 snapshot reports successful OpenAI and Anthropic probes and overall operational status. This is point-in-time evidence only; continue normal monitoring before making a sustained-health claim.
