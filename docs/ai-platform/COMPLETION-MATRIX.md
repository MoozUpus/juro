...
| Lawyer profile lifecycle and directory | `/api/platform/lawyer-profile`, `/api/platform/lawyer-profile/photo`, `/api/platform/lawyers` | PARTIAL | Worker `1721fec0-b8f6-451d-9e8a-4893204d8519`; authenticated RU directory showed only the approved-marketplace copy and zero approved profiles, with no overflow or console errors | `lib/platform/lawyer-marketplace.ts`, `app/api/platform/lawyer-profile/**`, `_platform/LawyerDirectoryClient.tsx` | Photo scan and approved-lawyer handoff smoke; public website marketplace | Critical | No | Create a synthetic controlled approved profile and verify its complete handoff flow | Type-check; `test`; `test:cloudflare`; lifecycle test; browser directory smoke | Staging privacy boundary deployed; full lifecycle remains partial |
