...
| Cases, plans and deadlines | cases and action-plan APIs | VERIFIED_WORKING | Existing lifecycle/action-plan tests plus an authenticated AI answer → accessible confirmation → case smoke; the saved synthetic answer created two persisted plan steps and two tasks | Verify direct citations appear in case source tab after egress repair | High | staging browser smoke passed |
