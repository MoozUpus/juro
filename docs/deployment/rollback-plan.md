...
| Platform Worker | v188, version ID `57387083-9f7f-4cd8-a9f2-84414f2604d6` | v187, version ID `65ce3f7f-3469-4c43-854c-d073309befed` | Verified platform regression, private-boundary failure, D1 probe misreporting, probe amplification, or new critical error rate |
...
1. Confirm the fault against the current v188 version and capture only bounded, non-sensitive diagnostics.
2. Prefer the Cloudflare version rollback to v187; do not rebuild an unrelated Git state.
3. Do not apply or alter a D1 migration during this rollback. v188 introduced no migration.
...
Neither the Sites nor v188 Worker rollback requires a database mutation. Do not delete user data, alter secrets, change DNS, or touch the excluded legislation/corpus systems as part of these rollback paths.
