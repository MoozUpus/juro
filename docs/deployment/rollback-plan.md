...
| Platform Worker | v206 `1ec688d4-e085-4aa9-a34d-df02b0c1ae1c` | v204 `12ae95e6-5eac-4f14-8257-a30dff56128d` | Verified platform regression, private-boundary failure, Individual navigation/touch-target/focus regression, auth/Turnstile layout regression, CSP regression, or new critical error rate |
...
1. Confirm the fault against the current v118 platform Worker or unchanged v116 website Worker version and capture only bounded, non-sensitive diagnostics.
2. Prefer the independently receipt-verified Cloudflare rollback to platform v204 `12ae95e6-5eac-4f14-8257-a30dff56128d`; if the unchanged public Worker separately regresses, use website v11 `fad80c80-ee92-44bb-93a3-e250ee314891`. Do not rebuild an unrelated Git state.
3. Do not apply or alter a D1 migration during this rollback. v118 introduced no migration.
...
Neither the Sites nor v118 Worker rollback requires a database mutation. Do not delete user data, alter secrets, change DNS, or touch the excluded legislation/corpus systems as part of these rollback paths.
