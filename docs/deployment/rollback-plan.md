# Rollback plan — Worker 164 / migrations 0159-0160 / Sites 86

## Application rollback

The active application version is
`3ba45422-86e9-4502-8ad2-8468bec57a78` (version 164), deployment
`46613e55-f973-4199-a825-e2c576ac63e1`. The immediate application rollback is
`e7c8ec49-bba6-4abd-ac00-89bfd1cd4acd` (version 163), deployment
`dc3efbec-6909-4f56-80ef-0d964cdea027`. Confirm the currently active version
...
API credit and is not a reason to roll back Worker 164. After any genuine
...
`/api/status` read. Worker 163 retains the Lawyer-host redirect, stable Lex
fingerprint, deterministic per-run digest, bounded unread-count query, safe
provider error classification and saved monitoring cadence. It does not contain
the dedicated monitoring-email outbox, so its API/UI must again present email as
unavailable. Record all preference cursors, monitoring-email job states and the
exact notification count before rollback, then compare them after two
five-minute cron windows and prefer a forward fix. Do not report overall
...
163 must not edit D1; the 0160 table remains dormant for the rollback Worker. A
