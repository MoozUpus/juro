...
This ADR historically rejected Vectorize as the sole official-corpus index because it lacked the mandatory BM25 channel. ADR 0006 instead pairs Vectorize with application-owned R2 BM25, so that rejection no longer applies to the complete custom pair. Qdrant/D1 remains available until the replacement's 90-day stability and restore gates pass; the incomplete AI Search experiment is retained only as evidence until its retirement checkpoint.
