...
- A bounded production D1 aggregate confirmed that the investor dataset still contains one active 90-day lawyer trial, one pending synthetic profile-deletion request and 12 records explicitly constrained to `provider=demo` and `is_simulation=1`. The final Chrome replay of the platform trial/deletion screen remains open because local navigation to `/ru/admin/lawyer-profiles` returns `ERR_BLOCKED_BY_CLIENT` before the request reaches the Worker; the D1 aggregate is not promoted to a visual pass.
...
- Complete Chrome zoom, live reduced-motion emulation, screen-share source selection, forced reconnect and the platform trial/deletion visual segment of the scripted investor rehearsal.
