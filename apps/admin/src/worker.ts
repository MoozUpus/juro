...
async function legalCorpus(request: Request, env: Env, session: string, notice?: string): Promise<Response> {
  ...
  const legacyRecoveryEnabled = data.environment === "staging"
    && env.LEGAL_CORPUS_QUEUE_PROCESSING_ENABLED === "true"
    && data.featureFlags.LEGAL_CORPUS_ENABLED
    && data.integrity.valid;
  const legacyRecoveryForms = legacyRecoveryEnabled
    ? data.failures
      .filter((item) => item.jobId && item.errorCode === "LEGAL_CORPUS_LANGUAGE_FAMILY_CONFLICT")
      .map((item) => `<form method="post" action="/legal-corpus/actions">${hidden}<input type="hidden" name="action" value="recover_legacy_ingestion"><input type="hidden" name="jobId" value="${escaped(item.jobId)}"><strong>Восстановить legacy language-family job</strong><p class="small">${escaped(item.language ?? "—")} · ${escaped(item.safeMessage)}</p><p class="small">Разрешено только в staging для точной записи; исходная failure ledger сохраняется.</p>${reason}<button>Восстановить legacy ingestion</button></form>`).join("")
    : "";
...
async function legalCorpusAction(request: Request, env: Env, session: string): Promise<Response> {
  ...
  if (
    (action !== "seed_discovery" && action !== "retry_discovery" && action !== "retry_ingestion"
      && action !== "recover_legacy_ingestion" && action !== "publish_owner_material" && action !== "withdraw_owner_material")
    ...
    || (action === "recover_legacy_ingestion" && (typeof jobId !== "string" || !/^legal-corpus:[A-Za-z0-9_-]{20,80}$/.test(jobId)))
