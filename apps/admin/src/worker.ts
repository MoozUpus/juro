...
function lawyerStatusLabel(status: string): string {
  return {
    ...
    pending_review: "Ожидает самопубликации",
    changes_requested: "Нужны исправления",
    public_approved: "Опубликован",
    rejected: "Отклонён",
...
function page(environment: Env["APP_ENV"], title: string, body: string, options: { notice?: string; role?: string } = {}): Response {
  ...
  const scriptNonce = crypto.randomUUID().replace(/-/gu, "");
  return new Response(`<!doctype html><html lang="ru" data-theme="system"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive"><title>${escaped(title)} · JURO</title><script nonce="${scriptNonce}">(()=>{const modes=new Set(["system","light","dark"]);const rawCookie=document.cookie.split("; ").find((item)=>item.startsWith("juro_theme="));let cookieMode=null;let storedMode=null;try{cookieMode=rawCookie?decodeURIComponent(rawCookie.slice("juro_theme=".length)):null;storedMode=localStorage.getItem("juro_theme")}catch{}const mode=modes.has(cookieMode)?cookieMode:modes.has(storedMode)?storedMode:"system";const apply=(next,persist)=>{document.documentElement.dataset.theme=next;if(persist){try{localStorage.setItem("juro_theme",next)}catch{}const domain=location.hostname.endsWith(".juro.uz")?"; Domain=.juro.uz":"";const secure=location.protocol==="https:"?"; Secure":"";document.cookie="juro_theme="+encodeURIComponent(next)+"; Path=/; Max-Age=31536000; SameSite=Lax"+domain+secure}document.querySelectorAll("[data-admin-theme]").forEach((button)=>button.setAttribute("aria-pressed",String(button.getAttribute("data-admin-theme")===next)))};apply(mode,false);addEventListener("DOMContentLoaded",()=>{apply(mode,false);document.querySelectorAll("[data-admin-theme]").forEach((button)=>button.addEventListener("click",()=>{const next=button.getAttribute("data-admin-theme");if(next&&modes.has(next))apply(next,true)}))})})();</script><style>
    ...
    :root,html[data-theme=light]{color-scheme:light;font-family:Manrope,"Segoe UI",sans-serif;background:#f8f6f2;color:#102333;--admin-bg:#f8f6f2;--admin-text:#102333;--admin-heading:#071a2e;--admin-gold:#be974f;--admin-gold-text:#916f2f;--admin-line:#dfe2df;--admin-muted:#647380;--admin-card:#fff;--admin-card-alt:#fbfaf7;--admin-card-gradient:#f8f6f2;--admin-table-head:#f5f6f4;--admin-input:#fff;--admin-button:#071a2e;--admin-button-text:#fff;--admin-danger:#812f2a;--admin-notice:#fff5dc;--admin-notice-text:#5b3a00;--admin-good:#176c4c;--admin-warn:#8a5a12;--admin-hover:#fffaf0;--admin-shadow:#071a2e0d;--admin-glow:#eadfc825}
    html[data-theme=dark]{color-scheme:dark;background:#081520;color:#e8edf2;--admin-bg:#081520;--admin-text:#e8edf2;--admin-heading:#f4f7f9;--admin-gold:#d8b36b;--admin-gold-text:#e4c885;--admin-line:#2b4150;--admin-muted:#aebdca;--admin-card:#0f2231;--admin-card-alt:#122838;--admin-card-gradient:#0b1d2a;--admin-table-head:#132a3a;--admin-input:#0b1d2a;--admin-button:#d8b36b;--admin-button-text:#071a2e;--admin-danger:#a94b43;--admin-notice:#362c17;--admin-notice-text:#f5dca0;--admin-good:#72d6ad;--admin-warn:#f0c979;--admin-hover:#1a3140;--admin-shadow:#00000035;--admin-glow:#be974f18}
    @media(prefers-color-scheme:dark){html[data-theme=system]{color-scheme:dark;background:#081520;color:#e8edf2;--admin-bg:#081520;--admin-text:#e8edf2;--admin-heading:#f4f7f9;--admin-gold:#d8b36b;--admin-gold-text:#e4c885;--admin-line:#2b4150;--admin-muted:#aebdca;--admin-card:#0f2231;--admin-card-alt:#122838;--admin-card-gradient:#0b1d2a;--admin-table-head:#132a3a;--admin-input:#0b1d2a;--admin-button:#d8b36b;--admin-button-text:#071a2e;--admin-danger:#a94b43;--admin-notice:#362c17;--admin-notice-text:#f5dca0;--admin-good:#72d6ad;--admin-warn:#f0c979;--admin-hover:#1a3140;--admin-shadow:#00000035;--admin-glow:#be974f18}}
    *{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 88% 4%,var(--admin-glow),transparent 32%),var(--admin-bg);color:var(--admin-text)}.shell{min-height:100vh;display:grid;grid-template-columns:17rem minmax(0,1fr)}aside{position:sticky;top:0;height:100vh;background:linear-gradient(180deg,#071a2e,#0b2b45);color:#fff;padding:1.8rem 1.35rem;display:flex;flex-direction:column;gap:1.5rem;box-shadow:14px 0 44px #071a2e14}main{min-width:0;max-width:96rem;width:100%;padding:clamp(2rem,4vw,4.5rem);margin:0 auto}main>h1,h2{color:var(--admin-heading)}main>h1{margin:0 0 .45rem;font-size:clamp(2rem,4vw,3.5rem);letter-spacing:-.045em;line-height:1}.brand{font-size:1.05rem;font-weight:850;letter-spacing:.16em;color:#d8b36b}.role{display:inline-flex;margin-top:.35rem;font-size:.74rem;border:1px solid #ffffff3d;border-radius:999px;padding:.32rem .58rem;color:#e8edf2}.nav{display:grid;gap:.35rem}.nav a{min-height:44px;color:#dbe7ef;text-decoration:none;padding:.65rem .75rem;border-left:3px solid transparent;border-radius:.65rem;display:flex;align-items:center;font-weight:700}.nav a:hover,.nav a:focus-visible{outline:2px solid #d8b36b;outline-offset:2px;background:#ffffff12;border-left-color:var(--admin-gold)}aside>p{color:#aebdca;line-height:1.55}.theme-control{display:grid;gap:.45rem}.theme-control>span{color:#aebdca;font-size:.72rem;font-weight:800;letter-spacing:.09em;text-transform:uppercase}.theme-options{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.3rem}.theme-options button{min-height:44px;padding:.45rem .25rem;border:1px solid #ffffff35;background:#ffffff0d;color:#dbe7ef;font-size:.7rem}.theme-options button[aria-pressed=true]{border-color:#d8b36b;background:#d8b36b;color:#071a2e}.theme-options button:hover,.theme-options button:focus-visible{outline:2px solid #d8b36b;outline-offset:2px}.eyebrow{margin:.4rem 0 .8rem;color:var(--admin-gold-text);font-size:.72rem;font-weight:850;letter-spacing:.15em;text-transform:uppercase}.session-summary{display:flex;flex-wrap:wrap;gap:.45rem 1rem;margin:.65rem 0 1.5rem;color:var(--admin-muted);font-size:.88rem}.session-summary strong{color:var(--admin-heading)}.panel{min-width:0;overflow:hidden;background:var(--admin-card);border:1px solid var(--admin-line);border-radius:1.1rem;padding:clamp(1rem,2vw,1.5rem);margin:1.25rem 0;box-shadow:0 18px 50px var(--admin-shadow)}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(12rem,1fr));gap:.85rem}.metric{min-height:8.2rem;padding:1.15rem;background:linear-gradient(145deg,var(--admin-card),var(--admin-card-gradient));border:1px solid var(--admin-line);border-radius:1rem;display:flex;flex-direction:column;justify-content:space-between}.metric span{color:var(--admin-text);font-size:.86rem;font-weight:750}.metric strong{display:block;color:var(--admin-heading);font-size:2.15rem;letter-spacing:-.04em}.metric small{color:var(--admin-muted);font-size:.72rem;line-height:1.4}.dashboard-grid{display:grid;grid-template-columns:minmax(0,1.45fr) minmax(18rem,.8fr);gap:1rem}.security-list{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.65rem;margin-top:1rem}.security-list div{border:1px solid var(--admin-line);border-radius:.8rem;padding:.85rem;background:var(--admin-card-alt)}.security-list strong{display:block;color:var(--admin-heading);font-size:.78rem}.security-list span{display:block;margin-top:.25rem;color:var(--admin-muted);font-size:.72rem;line-height:1.45}.quick-links{display:grid;gap:.55rem;margin-top:1rem}.quick-links a{display:flex;align-items:center;justify-content:space-between;min-height:48px;padding:.7rem .8rem;border:1px solid var(--admin-line);border-radius:.75rem;color:var(--admin-heading);font-weight:750;text-decoration:none}.quick-links a::after{content:'→';color:var(--admin-gold);font-size:1.1rem}.quick-links a:hover,.quick-links a:focus-visible{outline:3px solid var(--admin-gold);outline-offset:2px;background:var(--admin-hover)}.notice{background:var(--admin-notice);color:var(--admin-notice-text);border-left:.25rem solid var(--admin-gold);padding:.9rem 1rem;border-radius:.65rem}.filters{display:flex;flex-wrap:wrap;gap:.45rem;margin:1rem 0}.filters a{border:1px solid var(--admin-muted);border-radius:999px;color:var(--admin-heading);padding:.45rem .7rem;text-decoration:none}.filters a[aria-current=page]{background:var(--admin-button);border-color:var(--admin-button);color:var(--admin-button-text)}.filters a:hover,.filters a:focus-visible{outline:3px solid var(--admin-gold);outline-offset:2px}.scroll{max-width:100%;overflow:auto;border-radius:.75rem}table{width:100%;border-collapse:collapse}th{color:var(--admin-muted);background:var(--admin-table-head);font-size:.75rem;letter-spacing:.04em;text-transform:uppercase}th,td{text-align:left;padding:.8rem;border-bottom:1px solid var(--admin-line);vertical-align:top}button{min-height:44px;font:inherit;background:var(--admin-button);color:var(--admin-button-text);border:0;border-radius:.65rem;padding:.65rem .95rem;cursor:pointer;font-weight:750}button:hover,button:focus-visible{outline:3px solid var(--admin-gold);outline-offset:2px}input,textarea,select{font:inherit;width:100%;border:1px solid var(--admin-muted);border-radius:.55rem;padding:.65rem;background:var(--admin-input);color:var(--admin-text)}input[type=checkbox]{width:auto;margin-right:.35rem}label{display:grid;gap:.35rem;margin:.65rem 0}.actions{display:flex;gap:.5rem;flex-wrap:wrap}form.inline{display:inline}.danger{background:var(--admin-danger);color:#fff}.review-body{max-width:34rem;white-space:pre-wrap;overflow-wrap:anywhere}.compact{min-width:68rem;font-size:.78rem}.code{font:600 .72rem ui-monospace,monospace;overflow-wrap:anywhere}.ok{color:var(--admin-good)}.warn{color:var(--admin-warn)}.flags{display:grid;grid-template-columns:repeat(auto-fit,minmax(17rem,1fr));gap:.55rem}.flags span{display:flex;justify-content:space-between;gap:.6rem;padding:.65rem;border:1px solid var(--admin-line);border-radius:.6rem}.flags code{font-size:.7rem;overflow-wrap:anywhere}.small{color:var(--admin-muted);font-size:.72rem}.corpus-actions{display:grid;grid-template-columns:repeat(auto-fit,minmax(17rem,1fr));gap:.7rem}.corpus-actions form{padding:.85rem;border:1px solid var(--admin-line);border-radius:.75rem}.corpus-actions textarea{min-height:5rem}@media(max-width:64rem){.dashboard-grid{grid-template-columns:1fr}}@media(max-width:48rem){.shell{display:block}aside{position:static;height:auto;gap:.9rem;padding:1.1rem}.nav{grid-template-columns:repeat(2,minmax(0,1fr))}.nav a{min-width:0}.panel{padding:1rem}main{padding:1.2rem .9rem 3rem}.filters a{font-size:.78rem}.metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.security-list{grid-template-columns:1fr}}@media(max-width:26rem){.metrics{grid-template-columns:1fr}.flags,.corpus-actions{grid-template-columns:1fr}}
    </style></head><body><div class="shell"><aside><div><div class="brand">JURO ADMIN</div><p>Изолированная консоль ${environmentLabel}</p>${role}</div><nav class="nav"><a href="/">Обзор</a><a href="/legal-corpus">Правовой корпус</a><a href="/lawyers">Профили юристов</a><a href="/reviews">Отзывы</a><a href="${escaped("/logout")}">Сеанс</a></nav><div class="theme-control"><span>Тема оформления</span><div class="theme-options" role="group" aria-label="Тема оформления"><button type="button" data-admin-theme="system" aria-pressed="true">Система</button><button type="button" data-admin-theme="light" aria-pressed="false">Светлая</button><button type="button" data-admin-theme="dark" aria-pressed="false">Тёмная</button></div></div><p>Отдельная cookie. Каждое действие журналируется.</p></aside><main><h1>${escaped(title)}</h1>${notice}${body}</main></div></body></html>`, { headers: securityHeaders(scriptNonce) });
}
...
function securityHeaders(scriptNonce?: string): Headers {
  ...
  return new Headers({
    ...
    "content-security-policy": `default-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; font-src 'self'; img-src 'self' data:; script-src ${scriptSource}; style-src 'unsafe-inline'; upgrade-insecure-requests`,
...
async function dashboard(request: Request, env: Env, session: string): Promise<Response> {
  ...
  return page(env.APP_ENV, "Операционный обзор", `<p class="eyebrow">JURO · защищённый операционный контур</p><div class="session-summary"><span>Роль: <strong>${escaped(data.roles.join(", "))}</strong></span><span>Сеанс действует до <strong>${escaped(data.expiresAt)}</strong></span></div><section class="metrics" aria-label="Ключевые показатели"><div class="metric"><span>Ожидают самопубликации</span><strong>${data.counts.pendingLawyerProfiles}</strong><small>Профили без подтверждённого согласия на публикацию</small></div><div class="metric"><span>Опубликованные профили</span><strong>${data.counts.approvedLawyerProfiles}</strong><small>Доступны в публичном каталоге</small></div><div class="metric"><span>Активные заявки</span><strong>${data.counts.activeLawyerRequests}</strong><small>Текущие клиентские обращения</small></div><div class="metric"><span>События аудита</span><strong>${data.counts.adminAuditEvents}</strong><small>Неизменяемая история действий</small></div></section><div class="dashboard-grid"><section class="panel"><p class="eyebrow">Контроль доступа</p><h2>Изолированная административная граница</h2><p>Консоль использует отдельную host-only cookie и обращается к платформе только через service binding. Для текущей сессии обязательны активный TOTP и свежая MFA исходного JURO-сеанса.</p><div class="security-list"><div><strong>HOST-ONLY COOKIE</strong><span>Сессия не разделяется с пользовательскими поддоменами.</span></div><div><strong>SERVICE BINDING</strong><span>Внутренний API не публикуется во внешний контур.</span></div><div><strong>TOTP + FRESH MFA</strong><span>Привилегированный доступ ограничен коротким сеансом.</span></div></div></section><section class="panel"><p class="eyebrow">Быстрый доступ</p><h2>Операционные разделы</h2><nav class="quick-links" aria-label="Операционные разделы"><a href="/lawyers">Профили юристов</a><a href="/reviews">Модерация отзывов</a><a href="/legal-corpus">Правовой корпус</a></nav></section></div>`, { role: data.roles.join(" · ") });
}
...
async function lawyerList(request: Request, env: Env, session: string, notice?: string): Promise<Response> {
  ...
  const rows = result.body.profiles.map((profile) => {
    const moderation = profile.marketplaceStatus === "pending_review"
      ? `<form method="post" action="/lawyers/${encodeURIComponent(profile.id)}/moderate"><input type="hidden" name="_csrf" value="${escaped(csrfToken)}"><label>Причина запроса<textarea name="reason" required maxlength="2000" minlength="1"></textarea></label><div class="actions"><button name="decision" value="changes_requested">Запросить исправления</button></div></form>`
      : "";
  ...
  return page(env.APP_ENV, "Профили юристов", `<section class="panel"><p>Телефон и личный email не выдаются этой поверхности. Профили публикуются самим юристом после обязательных полей и согласия; контрольные действия требуют причины, роли и свежей MFA.</p><nav class="filters" aria-label="Статус профиля">${filters}</nav><div class="scroll"><table><thead><tr><th>Профиль</th><th>Город</th><th>Стаж</th><th>Изменён</th><th>Контроль и lifecycle</th></tr></thead><tbody>${rows || `<tr><td colspan="5">Нет профилей со статусом «${escaped(lawyerStatusLabel(selectedStatus))}».</td></tr>`}</tbody></table></div></section>`, { notice, role: "контроль профилей" });
}
...
async function legalCorpus(request: Request, env: Env, session: string, notice?: string): Promise<Response> {
  ...
  if (!result.response.ok || !result.body) {
    ...
    return page(env.APP_ENV, "Правовой корпус", "<p>Метрики корпуса временно недоступны. Перед повторной попыткой проверьте миграции и внутреннюю привязку Worker.</p>", { notice });
  }
  const data = result.body;
  const csrfToken = cookie(request, ADMIN_CSRF_COOKIE) ?? "";
  const metricLabels: Record<string, string> = {
    ...
    currentChunks: "Текущие фрагменты", indexedChunks: "Фрагменты в индексе",
    ...
    liveOrManualQueued: "Авто/ручные задания в очереди", failedDocuments: "Ошибки документов",
  };
  ...
  const seed = actionsEnabled && data.checkpoints.length === 0
    ? `<form method="post" action="/legal-corpus/actions">${hidden}<input type="hidden" name="action" value="seed_discovery"><input type="hidden" name="reason" value="Автоматическое первичное обнаружение из защищённой панели."><strong>Аварийное первичное обнаружение</strong><p class="small">Обычно не требуется: изолированный Worker создаёт контрольные точки автоматически. Причина первичного запуска записывается в защищённый журнал.</p><button>Создать контрольные точки</button></form>`
    : "";
  ...
  const ownerPublish = ownerPublishEnabled ? `<form method="post" action="/legal-corpus/actions">${hidden}<input type="hidden" name="action" value="publish_owner_material"><strong>Повторить публикацию завершённого анализа</strong><p class="small">Резервный технический повтор для ранее загруженного анализа, если автоматическая публикация не завершилась. Это не юридическое одобрение; действие требует свежую MFA.</p><label>ID анализа<input name="analysisId" required maxlength="180" pattern="[A-Za-z0-9:_-]+"></label><label>ID рабочего пространства<input name="workspaceId" required maxlength="180" pattern="[A-Za-z0-9:_-]+"></label><label>Публичное название<input name="title" required minlength="2" maxlength="300"></label><label>Язык<select name="language"><option value="ru">Русский</option><option value="uz-Latn">O‘zbekcha (lotin)</option><option value="uz-Cyrl">Ўзбекча (кирилл)</option><option value="en">English</option></select></label><label><span><input type="checkbox" name="rightsConfirmed" value="true" required> Подтверждаю права на глобальную публикацию материала в JURO.</span></label>${reason}<button>Повторить техническую публикацию</button></form>` : "";
  const ownerWithdraw = data.integrity.valid ? `<form method="post" action="/legal-corpus/actions">${hidden}<input type="hidden" name="action" value="withdraw_owner_material"><strong>Отозвать материал владельца</strong><p class="small">Отзыв доступен даже при выключенной загрузке корпуса. Материал немедленно исключается из поиска; исходные неизменяемые версии и аудиторские доказательства сохраняются.</p><label>ID документа корпуса<input name="documentId" required maxlength="180" pattern="[A-Za-z0-9:_-]+"></label>${reason}<button class="danger">Отозвать материал</button></form>` : "";
  ...
  const gateNotice = actionsEnabled
    ? `<p class="ok">Управление разрешено: оба функциональных флага корпуса включены, цепочка аудита действительна.</p>`
    : `<p class="notice">Управление заблокировано. Загрузка корпуса выключена либо цепочка аудита не прошла проверку. Просмотр метрик доступен.</p>`;
  ...
  return page(env.APP_ENV, "Правовой корпус", `
    ${gateNotice}
    <section class="panel"><h2>Техническое состояние</h2><p>Lex.uz: <strong>${escaped(data.lexHealth.state)}</strong> · проверено ${escaped(data.lexHealth.checkedAt ?? "—")} · цепочка аудита ${data.integrity.valid ? "действительна" : "нарушена"} (${escaped(data.integrity.checked)})</p><p>Qdrant: <strong class="${qdrant.status === "ready" ? "ok" : "warn"}">${escaped(qdrantLabel)}</strong> · настроен: ${qdrant.configured ? "да" : "нет"} · точек всего: ${escaped(qdrant.totalPoints ?? "—")} / актуальных: ${escaped(qdrant.currentPoints ?? "—")} · проверено ${escaped(qdrant.checkedAt)}${qdrant.errorCode ? ` · <code class="code warn">${escaped(qdrant.errorCode)}</code>` : ""}</p><section class="metrics">${metrics}</section></section>
    <section class="panel"><h2>Функциональные флаги</h2><div class="flags">${flags}</div></section>
    <section class="panel"><h2>Покрытие по категориям и языкам</h2><p class="small">Покрытие подтверждается, когда контрольная точка завершена, ожидаемое и фактически обнаруженное количества совпали, а каждый документ либо индексирован, либо имеет подтверждённый технический статус недоступности.</p><div class="scroll"><table class="compact"><thead><tr><th>Категория / язык</th><th>Найдено</th><th>Получено</th><th>Разобрано</th><th>Индексировано</th><th>Недоступно</th><th>Ожидается</th><th>Обновлено</th></tr></thead><tbody>${coverage || "<tr><td colspan=\"8\">Контрольные точки ещё не созданы.</td></tr>"}</tbody></table></div></section>
    <section class="panel"><h2>Управление</h2><p class="small">Первичное обнаружение запускается автоматически; пустое поле причины заполнять для него не нужно. Ручной повтор остаётся отдельным аудируемым действием. Добавление материала владельца требует роли администратора или юридического ревьюера, свежую MFA и подтверждение прав; оно не создаёт юридического заключения AI.</p><div class="corpus-actions">${ownerUpload}${ownerPublish}${ownerWithdraw}${seed}${checkpointForms}${failureForms}</div>${actionsEnabled && !checkpointForms && !failureForms ? "<p>Нет объектов для повторной попытки.</p>" : ""}</section>
    <section class="panel"><h2>Загрузки владельца</h2><p class="small">Статусы отражают фактическую последовательность: карантин → проверка на вредоносный код → извлечение и анализ → публикация. Текст файлов здесь не выводится.</p><div class="scroll"><table><thead><tr><th>Материал / ID анализа</th><th>Язык</th><th>Статус</th><th>Ошибка</th><th>Документ корпуса</th><th>Обновлено</th></tr></thead><tbody>${ownerUploads || "<tr><td colspan=\"6\">Загрузок ещё нет.</td></tr>"}</tbody></table></div></section>
    <section class="panel"><h2>Последние технические ошибки</h2><div class="scroll"><table><thead><tr><th>Код</th><th>Язык</th><th>Безопасное сообщение</th><th>Состояние повтора</th><th>Время</th></tr></thead><tbody>${failures || "<tr><td colspan=\"5\">Ошибок нет.</td></tr>"}</tbody></table></div></section>
    <section class="panel"><h2>Неизменяемый журнал корпуса</h2><div class="scroll"><table><thead><tr><th>Действие</th><th>Объект</th><th>Причина</th><th>Инициатор</th><th>Время</th></tr></thead><tbody>${events || "<tr><td colspan=\"5\">Ручных действий нет.</td></tr>"}</tbody></table></div></section>
  `, { notice, role: "суперадминистратор · правовой корпус" });
}
...
async function legalCorpusAction(request: Request, env: Env, session: string): Promise<Response> {
  ...
  const message = result.response.ok
    ...
    : result.body?.code === "LEGAL_CORPUS_ADMIN_DISABLED"
      ? "Загрузка корпуса выключена функциональными флагами; действие не выполнялось."
