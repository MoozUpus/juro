...
async function legalCorpus(request: Request, env: Env, session: string, notice?: string): Promise<Response> {
  ...
  const ownerPublish = ownerPublishEnabled ? `<form method="post" action="/legal-corpus/actions">${hidden}<input type="hidden" name="action" value="publish_owner_material"><strong>Опубликовать материал владельца</strong><p class="small">Только ваш завершённый analysis после malware scan и проверенного OCR. Действие требует одновременно super_admin, legal_reviewer и свежую MFA.</p><label>Analysis ID<input name="analysisId" required maxlength="180" pattern="[A-Za-z0-9:_-]+"></label><label>Workspace ID<input name="workspaceId" required maxlength="180" pattern="[A-Za-z0-9:_-]+"></label><label>Публичное название<input name="title" required minlength="2" maxlength="300"></label><label>Язык<select name="language"><option value="ru">Русский</option><option value="uz-Latn">O‘zbekcha (lotin)</option><option value="uz-Cyrl">Ўзбекча (кирилл)</option><option value="en">English</option></select></label><label><span><input type="checkbox" name="rightsConfirmed" value="true" required> Подтверждаю права на глобальную публикацию материала в JURO.</span></label><label><span><input type="checkbox" name="legalReviewConfirmed" value="true" required> Подтверждаю, что лично выполнил(а) юридическую проверку текста, языка и пригодности материала.</span></label>${reason}<button>Подтвердить и опубликовать</button></form>` : "";
  ...
  return page(env.APP_ENV, "Legal Corpus", `
    ...
    <section class="panel"><h2>Управление</h2><p class="small">Seed/retry — технические действия. Публикация owner material — отдельное персональное подтверждение legal_reviewer; оно не создаёт юридического заключения AI.</p><div class="corpus-actions">${ownerPublish}${ownerWithdraw}${seed}${checkpointForms}${failureForms}</div>${actionsEnabled && !checkpointForms && !failureForms ? "<p>Нет объектов для повторной попытки.</p>" : ""}</section>
...
async function legalCorpusAction(request: Request, env: Env, session: string): Promise<Response> {
  ...
  const legalReviewConfirmed = form.get("legalReviewConfirmed");
    ...
    : action === "retry_ingestion"
      ...
      : action === "publish_owner_material"
        ? { action, analysisId, workspaceId, title: typeof title === "string" ? title.trim() : "", language,
          rightsConfirmed: true, legalReviewConfirmed: true, reason: reason.trim() }
        : action === "withdraw_owner_material"
