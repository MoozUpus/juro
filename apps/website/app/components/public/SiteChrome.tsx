      ...
      <header className={styles.header} data-scrolled={scrolled || undefined} data-tone={tone}>
        ...
        <div className={styles.headerInner}>
          ...
          <div className={styles.actions}>
            <div className={narrowStyles.headerTheme}><PublicThemeSwitcher locale={locale} /></div>
            <div aria-label="Language" className={`${styles.languageSet} ${headerTouchStyles.languageSet} ${narrowStyles.languageSet}`}>{languages.map((target) => <Link aria-current={target === locale ? "page" : undefined} className={`${styles.language} ${headerTouchStyles.language}`} href={localeHref(target)} key={target}>{languageLabels[target]}</Link>)}</div>
