...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title, description, url: canonical, siteName: "JURO", type: "article", images: [{ url: "/juro-og.png", width: 1681, height: 909, alt: title }] },
    twitter: { card: "summary_large_image", title, description, images: ["/juro-og.png"] },
  };
}
...
export default async function EnglishLegalDocumentGuide({ params }: Props) {
  ...
  return <div className={styles.page} lang="en"><SiteHeader languageHref={`/ru/legal/${legalSlug}`} locale="en" /><main id="main-content" tabIndex={-1}><article className={styles.document}>
