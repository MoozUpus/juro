...
const copy = {
  ru: { back: "Каталог юристов", pending: "ПРОФИЛЬ ДОПОЛНЯЕТСЯ", newProfile: "НОВЫЙ ПРОФИЛЬ", independent: "Независимый специалист", pendingNotice: "Профиль уже публичный, но заявки станут доступны после заполнения обязательных профессиональных сведений.", about: "О специалисте", unknown: "Профессиональная информация уточняется.", education: "Образование", languages: "Языки", city: "Город", experience: "Опыт", price: "Стоимость", format: "Формат", request: "Оставить заявку", privacy: "Конфиденциальность до передачи дела", privacyLead: "Контактные данные не публикуются. Юрист получает ограниченное описание для проверки конфликта интересов; материалы дела передаются только после вашего отдельного подтверждения.", years: "лет" },
  uz: { back: "Yuristlar katalogi", pending: "PROFIL TO‘LDIRILMOQDA", newProfile: "YANGI PROFIL", independent: "Mustaqil mutaxassis", pendingNotice: "Profil ochiq, biroq so‘rovlar majburiy professional ma’lumotlar to‘ldirilgandan keyin ochiladi.", about: "Mutaxassis haqida", unknown: "Kasbiy ma’lumot aniqlashtirilmoqda.", education: "Ta’lim", languages: "Tillar", city: "Shahar", experience: "Tajriba", price: "Narx", format: "Format", request: "So‘rov qoldirish", privacy: "Ish topshirilishidan oldingi maxfiylik", privacyLead: "Kontakt ma’lumotlari ochiq e’lon qilinmaydi. Yurist manfaatlar to‘qnashuvini tekshirish uchun cheklangan tavsif oladi; ish materiallari faqat alohida tasdiqlashingizdan keyin beriladi.", years: "yil" },
  en: { back: "Professional catalogue", pending: "PROFILE DETAILS IN PROGRESS", newProfile: "NEW PROFILE", independent: "Independent professional", pendingNotice: "This profile is public, but requests become available after the required professional details are completed.", about: "About this professional", unknown: "Professional information is being clarified.", education: "Education", languages: "Languages", city: "City", experience: "Experience", price: "Price", format: "Format", request: "Request a consultation", privacy: "Privacy before case handoff", privacyLead: "Contact details are not published. The professional receives a limited description to check conflicts of interest; case materials are shared only after your separate confirmation.", years: "years" },
} as const;
...
export default async function LawyerProfilePage({ params }: Props) {
  ...
  return <div className={styles.page} lang={locale}><SiteHeader languageHref={`/ru/lawyers/${profileId}`} locale={locale} /><main id="main-content" tabIndex={-1}><article className={styles.profile}>
