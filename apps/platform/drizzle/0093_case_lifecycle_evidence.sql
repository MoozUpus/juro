-- Migration 0093: expand-only case lifecycle evidence.
-- The existing Worker can continue updating cases while this migration rolls
-- out. The new Worker writes the immutable ledger and the AFTER INSERT trigger
-- applies the corresponding current-state projection atomically.
ALTER TABLE `cases` ADD `lifecycle_revision` integer DEFAULT 0 NOT NULL;
--> statement-breakpoint
ALTER TABLE `cases` ADD `completed_at` text;
--> statement-breakpoint
ALTER TABLE `cases` ADD `completed_by_user_id` text;
--> statement-breakpoint
ALTER TABLE `cases` ADD `archived_by_user_id` text;
--> statement-breakpoint
CREATE TABLE `case_lifecycle_events` (
	...
	CONSTRAINT `case_lifecycle_hash_check` CHECK (
		`previous_hash` GLOB replace(lower(hex(zeroblob(32))),'0','[0-9a-f]')
		AND `event_hash` GLOB replace(lower(hex(zeroblob(32))),'0','[0-9a-f]')
	)
);
--> statement-breakpoint
