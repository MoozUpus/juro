...
CREATE UNIQUE INDEX `analysis_report_exports_idempotency_uidx` ON `analysis_report_exports` (`idempotency_key`);--> statement-breakpoint
CREATE UNIQUE INDEX `analysis_report_exports_r2_key_uidx` ON `analysis_report_exports` (`r2_key`);--> statement-breakpoint
CREATE INDEX `analysis_report_exports_analysis_idx` ON `analysis_report_exports` (`analysis_id`,`created_at`);--> statement-breakpoint
CREATE INDEX `analysis_report_exports_workspace_idx` ON `analysis_report_exports` (`workspace_id`,`created_at`);--> statement-breakpoint
CREATE INDEX `analysis_report_exports_status_idx` ON `analysis_report_exports` (`status`,`updated_at`);--> statement-breakpoint
CREATE TRIGGER analysis_report_exports_insert_guard
BEFORE INSERT ON analysis_report_exports
FOR EACH ROW BEGIN
  SELECT RAISE(ABORT, 'analysis_report_export_source_mismatch') WHERE NOT EXISTS (
...
END;
--> statement-breakpoint
CREATE TRIGGER analysis_report_exports_update_guard
BEFORE UPDATE ON analysis_report_exports
FOR EACH ROW BEGIN
  SELECT RAISE(ABORT, 'analysis_report_export_identity_immutable') WHERE NEW.id <> OLD.id
  ...
  SELECT RAISE(ABORT, 'analysis_report_export_transition_invalid') WHERE NOT (
  ...
  SELECT RAISE(ABORT, 'analysis_report_export_completion_invalid') WHERE NEW.status = 'completed' AND (
  ...
  SELECT RAISE(ABORT, 'analysis_report_export_incomplete_has_artifact') WHERE NEW.status <> 'completed' AND (
