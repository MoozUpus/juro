-- Migration 0113: append-only, content-free AI latency/SLO telemetry.
-- This table must never store prompts, answers, documents, URLs, e-mail
-- addresses, user/workspace identifiers, provider payloads, or credentials.
-- correlation_hash is derived from an opaque request correlation UUID and is
-- intentionally not a foreign key to an account-owned record.
