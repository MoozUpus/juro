-- Migration 0085: operator-requested queue redrives with immutable evidence.
CREATE TABLE `operational_job_redrive_events` (
	...
	CONSTRAINT `operational_job_redrive_previous_hash_check` CHECK (`previous_event_hash` IS NULL OR `previous_event_hash` GLOB replace(hex(zeroblob(32)),'0','[A-F0-9]')),
	CONSTRAINT `operational_job_redrive_event_hash_check` CHECK (`event_hash` GLOB replace(hex(zeroblob(32)),'0','[A-F0-9]'))
);
--> statement-breakpoint
CREATE UNIQUE INDEX `operational_job_redrive_source_version_uidx`
ON `operational_job_redrive_events` (`source_job_id`,`version`);
--> statement-breakpoint
CREATE UNIQUE INDEX `operational_job_redrive_event_hash_uidx`
ON `operational_job_redrive_events` (`event_hash`);
--> statement-breakpoint
CREATE INDEX `operational_job_redrive_environment_created_idx`
ON `operational_job_redrive_events` (`environment`,`created_at` DESC);
--> statement-breakpoint
CREATE TRIGGER `operational_job_redrive_actor_guard`
BEFORE INSERT ON `operational_job_redrive_events`
WHEN NOT EXISTS (SELECT 1 FROM `user_profiles` WHERE `id`=NEW.`actor_user_id`)
BEGIN
	SELECT RAISE(ABORT, 'OPERATIONAL_JOB_REDRIVE_ACTOR_UNAVAILABLE');
END;
--> statement-breakpoint
CREATE TRIGGER `operational_job_redrive_sequence_guard`
BEFORE INSERT ON `operational_job_redrive_events`
WHEN NEW.`version` <> COALESCE((
	SELECT MAX(`version`) + 1 FROM `operational_job_redrive_events`
	WHERE `source_job_id`=NEW.`source_job_id`
), 1)
OR COALESCE(NEW.`previous_event_hash`,'') <> COALESCE((
	SELECT `event_hash` FROM `operational_job_redrive_events`
	WHERE `source_job_id`=NEW.`source_job_id`
	ORDER BY `version` DESC LIMIT 1
),'')
BEGIN
	SELECT RAISE(ABORT, 'OPERATIONAL_JOB_REDRIVE_SEQUENCE_CONFLICT');
END;
--> statement-breakpoint
CREATE TRIGGER `operational_job_redrive_projection_guard`
BEFORE INSERT ON `operational_job_redrive_events`
WHEN NOT EXISTS (
	SELECT 1 FROM `job_runs` AS j
	JOIN `job_outbox` AS o ON o.`id`=NEW.`outbox_id` AND o.`idempotency_key`=j.`idempotency_key`
	WHERE j.`id`=NEW.`source_job_id`
	  ...
	  AND (
	    ...
	    OR j.`error_code` IN (
	      ...
	      'JOB_TRANSIENT_FAILURE','JOB_LEASE_LOST','DOCUMENT_ANALYSIS_PROVIDER_UNAVAILABLE',
	      'DOCUMENT_ANALYSIS_PERSISTENCE_FAILED','USER_DOCUMENT_INDEX_FAILED',
	      'OCR_PROVIDER_UNAVAILABLE','OCR_PERSISTENCE_FAILED','DOCUMENT_EXPORT_OBJECT_FAILED',
...
END;
--> statement-breakpoint
CREATE TRIGGER `operational_job_redrive_apply`
AFTER INSERT ON `operational_job_redrive_events`
BEGIN
	UPDATE `job_runs`
	SET `status`='retrying',`lease_owner`=NULL,`lease_expires_at`=NULL,
	    `next_attempt_at`=NEW.`created_at`,`finished_at`=NULL,`updated_at`=NEW.`created_at`
	WHERE `id`=NEW.`source_job_id`;
	UPDATE `job_outbox`
	SET `status`='pending',`available_at`=NEW.`created_at`,`lease_owner`=NULL,
	    `lease_expires_at`=NULL,`next_attempt_at`=NULL,`dispatched_at`=NULL,
	    `error_code`=NULL,`updated_at`=NEW.`created_at`
	WHERE `id`=NEW.`outbox_id`;
END;
--> statement-breakpoint
CREATE TRIGGER `operational_job_redrive_no_update`
BEFORE UPDATE ON `operational_job_redrive_events`
BEGIN
	SELECT RAISE(ABORT, 'OPERATIONAL_JOB_REDRIVE_IMMUTABLE');
END;
--> statement-breakpoint
CREATE TRIGGER `operational_job_redrive_no_delete`
BEFORE DELETE ON `operational_job_redrive_events`
BEGIN
	SELECT RAISE(ABORT, 'OPERATIONAL_JOB_REDRIVE_IMMUTABLE');
END;
