-- Migration 0119: extend the existing append-only operational switch ledger
-- with provider- and Lex-discovery-specific kill switches.
CREATE TABLE `operational_feature_flag_versions_next` (
	...
	CONSTRAINT `operational_feature_key_check` CHECK (`feature_key` IN ('ai_chat','ai_openai_primary','ai_anthropic_fallback','ai_lex_web_discovery','document_analysis_upload','lawyer_handoff','voice_mode')),
