-- Replace the fixed-size GLOB expression from 0087. SQLite/D1 rejects its
-- expanded pattern as too complex before it evaluates a valid 64-character hash.
PRAGMA foreign_keys=OFF;
--> statement-breakpoint
CREATE TABLE `__new_ai_quality_review_events` (
...
--> statement-breakpoint
INSERT INTO `__new_ai_quality_review_events`
SELECT * FROM `ai_quality_review_events`;
--> statement-breakpoint
DROP TABLE `ai_quality_review_events`;
--> statement-breakpoint
ALTER TABLE `__new_ai_quality_review_events` RENAME TO `ai_quality_review_events`;
--> statement-breakpoint
...
BEFORE INSERT ON `ai_quality_review_events`
WHEN NEW.`request_action`='view' AND NOT EXISTS (SELECT 1 FROM `ai_feedback` WHERE `id`=NEW.`feedback_id` AND `updated_at`=NEW.`feedback_updated_at`)
BEGIN
