...
type Proposal = { id: string; lawyerRequestId: string; lawyerProfileId: string; lawyerUserId: string; lawyerBaseAmountMinor: number; titleRu: string; titleUz: string; titleEn: string | null; acceptanceId: string; agreementVersion: string; agreementSha256: string };
...
async function approvedProposal(db: D1Database, actor: Actor, proposalId: string): Promise<Proposal> {
  const proposal = await db.prepare(`SELECT p.id,p.lawyer_request_id AS lawyerRequestId,p.lawyer_profile_id AS lawyerProfileId,p.lawyer_user_id AS lawyerUserId,p.lawyer_base_amount_minor AS lawyerBaseAmountMinor,p.title_ru AS titleRu,p.title_uz AS titleUz,p.title_en AS titleEn,a.id AS acceptanceId,a.agreement_version AS agreementVersion,a.agreement_sha256 AS agreementSha256
...
export async function createMarketplaceServiceCheckout(db: D1Database, actor: Actor, input: { proposalId: string; requestId: string }, now = new Date()): Promise<CheckoutOrderView> {
  ...
  try { await db.batch([
    ...
    db.prepare("INSERT INTO order_items (id,order_id,item_type,reference_type,reference_id,title_ru,title_uz,title_en,quantity,unit_amount_minor,base_amount_minor,tax_amount_minor,total_amount_minor,currency,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(crypto.randomUUID(), orderId, "LEGAL_SERVICE", "legal_service_proposal", proposal.id, proposal.titleRu, proposal.titleUz, proposal.titleEn, 1, proposal.lawyerBaseAmountMinor, proposal.lawyerBaseAmountMinor, pricing.lawyerVatAmountMinor + pricing.juroVatAmountMinor, pricing.clientTotalMinor, "UZS", at),
