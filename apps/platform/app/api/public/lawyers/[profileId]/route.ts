...
export async function GET(_request: Request, context: Context) {
  ...
  const aggregates = await db.prepare(
    `SELECT r.lawyer_profile_id AS lawyerProfileId,COUNT(*) AS reviewCount,AVG(r.overall_rating) AS overallAverage,
      AVG(r.speed_rating) AS speedAverage,AVG(r.quality_rating) AS qualityAverage,AVG(r.communication_rating) AS communicationAverage
     FROM lawyer_reviews r JOIN lawyer_review_moderation m ON m.review_id=r.id AND m.decision='approved'
     WHERE r.lawyer_profile_id=? AND r.status='approved' GROUP BY r.lawyer_profile_id`,
  ).bind(parsedId.data).all<{ lawyerProfileId: string; reviewCount: number; overallAverage: number; speedAverage: number; qualityAverage: number; communicationAverage: number }>();
  const reviews = await db.prepare(
    `SELECT r.id AS reviewId,r.lawyer_profile_id AS lawyerProfileId,r.overall_rating AS overallRating,
      ...
      (SELECT COALESCE(replyModeration.moderated_body,reply.body) FROM lawyer_review_replies reply
       ...
       WHERE reply.review_id=r.id AND reply.status='approved' ORDER BY reply.version DESC LIMIT 1) AS replyBody,
      (SELECT reply.created_at FROM lawyer_review_replies reply
       JOIN lawyer_review_reply_moderation replyModeration ON replyModeration.reply_id=reply.id AND replyModeration.decision='approved'
       WHERE reply.review_id=r.id AND reply.status='approved' ORDER BY reply.version DESC LIMIT 1) AS replyCreatedAt
     FROM lawyer_reviews r JOIN lawyer_review_moderation m ON m.review_id=r.id AND m.decision='approved'
     WHERE r.lawyer_profile_id=? AND r.status='approved' ORDER BY m.created_at DESC,r.id DESC LIMIT 3`,
  ).bind(parsedId.data).all<{ reviewId: string; lawyerProfileId: string; overallRating: number; body: string | null; createdAt: string; replyBody: string | null; replyCreatedAt: string | null }>();
  return Response.json({ lawyer: projectPublicLawyerDirectory(lawyers.results, aggregates.results, reviews.results)[0] }, {
    headers: { "cache-control": "public, max-age=60, s-maxage=60, stale-while-revalidate=300", "x-content-type-options": "nosniff" },
  });
}
