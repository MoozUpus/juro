...
/** Deliberately unauthenticated, field-allowlisted projection for juro.uz. */
export async function GET() {
  ...
  const aggregates = await db.prepare(
    `SELECT r.lawyer_profile_id AS lawyerProfileId,COUNT(*) AS reviewCount,
      ...
      AVG(r.quality_rating) AS qualityAverage,AVG(r.communication_rating) AS communicationAverage
  ...
  return Response.json({ lawyers: projectPublicLawyerDirectory(lawyers.results, aggregates.results, reviews.results) }, {
