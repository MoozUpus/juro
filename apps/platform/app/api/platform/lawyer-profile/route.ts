...
async function accountIsLawyer(userId: string) {
  ...
  ).bind(userId).first<{ permitted: number }>();
}
...
async function ownProfile(userId: string) {
  ...
  ).bind(userId).first<LawyerProfile>();
}
...
function changed(current: EditableProfile, next: EditableProfile): boolean {
  return current.displayName !== next.displayName
    ...
    || JSON.stringify(current.consultationFormats) !== JSON.stringify(next.consultationFormats);
}
...
export const POST = withApiErrors(async function POST(request: Request) {
  ...
  const user = await requireApiUser();
  const parsed = await parseJsonRequest(request, lawyerProfileCreateSchema, 12_288);
  const locale = parsed.ok ? parsed.data.locale : "ru";
  if (!parsed.ok) {
    return response({ code: "INVALID_INPUT", error: lawyerProfileError(locale, "INVALID_INPUT") }, parsed.error === "payload_too_large" ? 413 : 400);
  }
  if (!await accountIsLawyer(user.id)) {
    return response({ code: "PROFILE_FORBIDDEN", error: lawyerProfileError(locale, "PROFILE_FORBIDDEN") }, 403);
  }
...
export const PATCH = withApiErrors(async function PATCH(request: Request) {
  ...
  assertSafeWrite(request);
  const user = await requireApiUser();
  const parsed = await parseJsonRequest(request, lawyerProfileUpdateSchema, 12_288);
  ...
  const next: EditableProfile = {
    ...
    experienceYears: value.experienceYears === undefined ? current.experienceYears : value.experienceYears,
    priceDescription: value.priceDescription === undefined ? current.priceDescription : value.priceDescription,
    availabilityStatus: value.availabilityStatus ?? current.availabilityStatus,
    nextAvailableAt: value.nextAvailableAt === undefined ? current.nextAvailableAt : value.nextAvailableAt,
    advocateStatus: profile.advocateStatus === "verified"
      ? "verified"
    ...
    education: value.education === undefined ? current.education : value.education,
    consultationFormats: value.consultationFormats ?? current.consultationFormats,
  };
  if (!changed(current, next)) return response({ profile: serialize(profile) });

  const marketplaceStatus = marketplaceStatusAfterProfileEdit(completion(profile, next));
  const missingRequiredFields = missingLawyerMarketplaceFields(completion(profile, next));
  const workspace = await workspaceForUser(user);
  const now = isoNow();
  const db = requireD1();
  ...
  const statements = [
    ...
    db.prepare(
      `INSERT INTO workspace_audit_events
       (id,workspace_id,actor_user_id,entity_type,entity_id,action,metadata_json,created_at)
       SELECT ?,?,?,'lawyer_profile',?,'lawyer_profile_reapproval_requested',?,?
       WHERE EXISTS (
