...
import { isLawyerMarketplaceProfileComplete } from "../../../../../../lib/platform/lawyer-marketplace";
import { z } from "zod";

type Context = { params: Promise<{ profileId: string }> };
type PendingProfile = { id: string; workspaceId: string | null; profileRevision: number; displayName: string; specialtiesJson: string; languagesJson: string; experienceYears: number | null; priceDescription: string | null; availabilityStatus: string; nextAvailableAt: string | null; advocateStatus: string; firmName: string | null; bio: string | null; city: string | null; region: string | null; education: string | null; consultationFormatsJson: string; profilePhotoKey: string | null; hasPhone: number };
...
export async function PATCH(request: Request, context: Context) {
  ...
  })) return Response.json({ code: "PROFILE_INCOMPLETE" }, { status: 409 });
  const now = isoNow();
  const moderationId = crypto.randomUUID();
  const auditId = crypto.randomUUID();
  const profileSha256 = await sha256(JSON.stringify({ id: profile.id, revision: profile.profileRevision, displayName: profile.displayName, specialtiesJson: profile.specialtiesJson, languagesJson: profile.languagesJson, experienceYears: profile.experienceYears, priceDescription: profile.priceDescription, availabilityStatus: profile.availabilityStatus, nextAvailableAt: profile.nextAvailableAt, advocateStatus: profile.advocateStatus, firmName: profile.firmName, bio: profile.bio, city: profile.city, region: profile.region, education: profile.education, consultationFormatsJson: profile.consultationFormatsJson, profilePhotoKey: profile.profilePhotoKey }));
  try {
    const results = await db.batch([
      ...
      db.prepare("UPDATE lawyer_profiles SET marketplace_status=?,updated_at=? WHERE id=? AND profile_revision=? AND status=?").bind(parsed.data.decision === "approved" ? "public_approved" : "rejected", now, profile.id, profile.profileRevision, parsed.data.decision === "approved" ? "public_approved" : "rejected"),
    ]);
    if (Number(results[0]?.meta.changes ?? 0) !== 1 || Number(results[1]?.meta.changes ?? 0) !== 1 || Number(results[2]?.meta.changes ?? 0) !== 1) return Response.json({ code: "PROFILE_UNAVAILABLE" }, { status: 409 });
  } catch {
