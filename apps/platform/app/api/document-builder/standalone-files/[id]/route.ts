...
async function ownedFile(id: string, userId: string) {
  return requireD1().prepare(
    "SELECT id, r2_key AS r2Key, file_name AS fileName, mime_type AS mimeType, size_bytes AS sizeBytes, archived_at AS archivedAt FROM document_files WHERE id = ? AND owner_user_id = ? AND kind = 'standalone_signed_pdf' LIMIT 1",
  ).bind(id, userId).first<{ id: string; r2Key: string; fileName: string; mimeType: string; sizeBytes: number; archivedAt: string | null }>();
}
...
export async function GET(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const file = await ownedFile(id, user.id);
...
export async function PATCH(request: Request, context: Context): Promise<Response> {
  try {
    ...
    if (body.action === "rename") {
      ...
      await db.prepare("UPDATE document_files SET file_name = ?, updated_at = ? WHERE id = ? AND owner_user_id = ?").bind(fileName, now, id, user.id).run();
      return jsonResponse({ fileName });
    }
    if (body.action === "archive") {
      await db.batch([
        db.prepare("UPDATE document_files SET archived_at = ?, updated_at = ? WHERE id = ? AND owner_user_id = ?").bind(now, now, id, user.id),
    ...
    if (body.action === "restore") {
      await db.prepare("UPDATE document_files SET archived_at = NULL, updated_at = ? WHERE id = ? AND owner_user_id = ?").bind(now, id, user.id).run();
...
export async function DELETE(request: Request, context: Context): Promise<Response> {
  try {
    ...
    const file = await ownedFile(id, user.id);
    if (!file) return notFound("Подписанный PDF не найден.");
    await requireD1().prepare("DELETE FROM document_files WHERE id = ? AND owner_user_id = ?").bind(id, user.id).run();
