...
export function FinalSuccess({ files, libraryPath, onDownload, onPrint, onConsultation }: { files: { docx: GeneratedFile; pdf: GeneratedFile; zip: GeneratedFile }; libraryPath: string; onDownload: (file: GeneratedFile) => void; onPrint: () => void; onConsultation: () => void }) {
  ...
  return <section className="dbt-success">
    ...
    <div className="dbt-related"><h2>Связанные документы</h2><div>{related.map((title) => <article key={title}><FileText size={22}/><strong>{title}</strong><Link href={libraryPath} title="Выбрать документ в библиотеке JURO">Создать документ</Link></article>)}</div></div>
