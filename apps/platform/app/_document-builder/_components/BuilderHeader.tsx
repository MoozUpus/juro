...
export function BuilderHeader({ user, signInPath, compact = false, variant = "standalone" }: { user: BuilderUser | null; signInPath?: string; compact?: boolean; variant?: "standalone" | "embedded" }) {
  ...
  return <header className={`dbt-header${embedded ? " dbt-header-embedded" : ""}`}>
    {embedded ? <div className="dbt-embedded-title"><FilePenLine aria-hidden="true"/><span><small>JURO</small><strong>{uz ? "Hujjatlar" : "Документы"}</strong></span></div> : <Link className="dbt-brand" href={paths.builder} aria-label={uz ? "JURO — Hujjat yaratish" : "JURO — Создать документ"}>
    ...
    {!embedded && <nav aria-label={uz ? "Hujjat konstruktori navigatsiyasi" : "Навигация конструктора документов"}>
