...
export function DocumentAssetsPanel({ documentId, onDocumentChange }: { documentId: string; onDocumentChange: (document: StoredDocument) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    apiFetch<CollaborationSnapshot>(`/api/document-builder/documents/${documentId}/collaboration`).then(setCollaboration).catch(() => undefined);
  ...
  const changeAttachment = async (attachmentId: string, visibleToCollaborator: boolean) => {
    await apiFetch(`/api/document-builder/documents/${documentId}/attachments`, { method: "PATCH", body: JSON.stringify({ attachmentId, visibleToCollaborator }) }); await load();
  };
  const deleteAttachment = async (attachmentId: string) => {
    await apiFetch(`/api/document-builder/documents/${documentId}/attachments`, { method: "DELETE", body: JSON.stringify({ attachmentId }) }); await load();
  };
  const analyzeAttachment = async (attachment: Attachment) => {
    setBusy(`analysis-${attachment.id}`); setAnalysis(null); setError("");
    try {
      const result = await apiFetch<{ status: string; message?: string; result?: Record<string, unknown> }>("/api/document-builder/attachment-analysis", { method: "POST", body: JSON.stringify({ documentId, attachmentId: attachment.id, questionnaireSummary: document?.autoContent.slice(0, 10_000) }) });
  ...
  const createShare = async () => {
    setBusy("share");
    try { const result = await apiFetch<{ url: string; expiresAt: string }>(`/api/document-builder/documents/${documentId}/share`, { method: "POST", body: JSON.stringify({ action: "create" }) }); setShare(result); }
  ...
  return <section className="dbt-assets">
    ...
    {signedFile && <div className="dbt-signed-file"><div><ShieldCheck size={24}/><span><strong>Подписанный PDF загружен</strong><small>Этот файл нельзя заменить.</small></span></div><a href={`/api/document-builder/documents/${documentId}/files/${signedFile.id}?inline=1`} target="_blank" rel="noreferrer"><Eye size={17}/>Открыть подписанную версию</a></div>}
