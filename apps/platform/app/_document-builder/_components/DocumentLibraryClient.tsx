...
function TemplateCard({ document, language, paths }: { document: DocumentLibraryItem; language: BuilderLanguage; paths: BuilderNavigationPaths }) {
  ...
  return <article className={`dbt-template-card ${available ? "published" : "review"}`}>
    ...
    {available ? <Link href={paths.template(document.categorySlug, document.code)}>{language === "uz" ? "Hujjat yaratish" : "Создать документ"}<ArrowRight size={17}/></Link> : <span className="dbt-template-disabled"><ShieldCheck size={16}/>{language === "uz" ? "Vaqtincha mavjud emas" : "Временно недоступен"}</span>}
  </article>;
}
...
export function DocumentLibraryClient({ categories, documents, activeCategory, signedIn = false }: { categories: readonly DocumentCategory[]; documents: readonly DocumentLibraryItem[]; activeCategory?: DocumentCategory; signedIn?: boolean }) {
  ...
  const changeLanguage = (next: BuilderLanguage) => {
    ...
    if (paths.locale && paths.locale !== next) router.push(paths.switchLocale(next));
  };
  ...
  return <div className="dbt-library-page">
    ...
    {!activeCategory && <section className="dbt-workspace-links" aria-label={language === "uz" ? "Hujjatlar bilan ishlash" : "Работа с документами"}>
      <Link href={paths.template("debt", "0602001")}><FilePenLine size={21}/><span><strong>{language === "uz" ? "Tilxat konstruktori" : "Конструктор расписки"}</strong><small>{language === "uz" ? "Maxsus bosqichma-bosqich shakl" : "Специализированная пошаговая форма"}</small></span><ArrowRight size={18}/></Link>
      <Link href={paths.documents}><FileCheck2 size={21}/><span><strong>{language === "uz" ? "Mening hujjatlarim" : "Мои документы"}</strong><small>{signedIn ? (language === "uz" ? "Qoralamalar va tayyor fayllar" : "Черновики и готовые файлы") : (language === "uz" ? "Kirish talab etiladi" : "Потребуется вход")}</small></span><ArrowRight size={18}/></Link>
      <Link href={paths.notifications}><Bell size={21}/><span><strong>{language === "uz" ? "Takliflar va harakatlar" : "Приглашения и действия"}</strong><small>{language === "uz" ? "Kelishuvni kutayotgan hujjatlar" : "Документы, ожидающие согласования"}</small></span><ArrowRight size={18}/></Link>
      <Link href={paths.contacts}><UsersRound size={21}/><span><strong>{language === "uz" ? "Tomonlar va kontaktlar" : "Стороны и контакты"}</strong><small>{language === "uz" ? "Ishtirokchilar ma’lumotlarini qayta ishlatish" : "Повторное использование данных участников"}</small></span><ArrowRight size={18}/></Link>
    </section>}
    ...
    <div className="dbt-library-tools"><label><Search size={18}/><input value={search} onChange={(event) => { setSearch(event.target.value); setLimit(TEMPLATE_PAGE_SIZE); }} placeholder={language === "uz" ? "Nomi yoki kodi bo‘yicha qidirish" : "Поиск по названию или коду"} aria-label={language === "uz" ? "Hujjat nomi yoki kodi bo‘yicha qidirish" : "Поиск документа по названию или коду"}/></label><select value={status} onChange={(event) => { setStatus(event.target.value as typeof status); setLimit(TEMPLATE_PAGE_SIZE); }} aria-label={language === "uz" ? "Holat" : "Статус"}><option value="all">{language === "uz" ? "Barcha holatlar" : "Все статусы"}</option><option value="verified">{language === "uz" ? "Tekshirilgan" : "Проверены"}</option><option value="beta">{language === "uz" ? "Beta · tekshiruvda" : "Бета · на проверке"}</option></select></div>
