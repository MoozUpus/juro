...
export function NotificationsClient({ user, signInPath }: { user: ChatGPTUser; signInPath: string }) {
  ...
  const load = () => apiFetch<{ notifications: NotificationRecord[] }>("/api/document-builder-test/notifications").then((result) => setItems(result.notifications)).catch((caught: Error) => setError(caught.message));
  useEffect(() => { void load(); }, []);
  const mark = async (id?: string) => { await apiFetch("/api/document-builder-test/notifications", { method: "PATCH", body: JSON.stringify(id ? { id } : { all: true }) }); await load(); };
