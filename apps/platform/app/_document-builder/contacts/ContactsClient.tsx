...
export function ContactsClient({ user, signInPath }: { user: ChatGPTUser; signInPath: string }) {
  ...
  const load = () => apiFetch<{ contacts: ContactRecord[] }>("/api/document-builder-test/contacts").then((result) => setContacts(result.contacts)).catch((caught: Error) => setError(caught.message));
  useEffect(() => { void load(); }, []);
  const update = (field: keyof ContactForm, value: string) => setForm((current) => ({ ...current, [field]: value }));
  const submit = async (event: FormEvent) => {
    event.preventDefault(); setError("");
    try {
      await apiFetch(editingId ? `/api/document-builder-test/contacts/${editingId}` : "/api/document-builder-test/contacts", { method: editingId ? "PUT" : "POST", body: JSON.stringify(form) });
