...
export function DocumentsClient({ user, signInPath }: { user: ChatGPTUser; signInPath: string }) {
  ...
  return <div className="dbt-root"><BuilderHeader user={user} signInPath={signInPath}/><div className="dbt-documents-page">
    <header className="dbt-page-title"><div><span><Files size={22}/></span><div><h1>Мои документы</h1><p>{total} {total === 1 ? "документ" : "документов и файлов"}</p></div></div><Link href={paths.library}><FilePlus2 size={18}/>Создать документ</Link></header>
