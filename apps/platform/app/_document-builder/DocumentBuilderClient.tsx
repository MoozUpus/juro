...
import type { GeneratedFile } from "./_components/FinalSuccess";
...
const DeferredCollaborationPanel = lazy(() => import("./_components/CollaborationPanel")
  .then(({ CollaborationPanel }) => ({ default: CollaborationPanel })));
const DeferredDocumentAssetsPanel = lazy(() => import("./_components/DocumentAssetsPanel")
  .then(({ DocumentAssetsPanel }) => ({ default: DocumentAssetsPanel })));
const DeferredFinalSuccess = lazy(() => import("./_components/FinalSuccess")
  .then(({ FinalSuccess }) => ({ default: FinalSuccess })));
const DeferredManualEditor = lazy(() => import("./_components/ManualEditor")
  .then(({ ManualEditor }) => ({ default: ManualEditor })));
const DeferredBuilderAnalysisLauncher = lazy(() => import("./_components/BuilderAnalysisLauncher")
  .then(({ BuilderAnalysisLauncher }) => ({ default: BuilderAnalysisLauncher })));
const DeferredBuilderVersionHistory = lazy(() => import("./_components/BuilderVersionHistory")
  .then(({ BuilderVersionHistory }) => ({ default: BuilderVersionHistory })));

function DeferredPanelFallback({ label }: { label: string }) {
  return <div className="dbt-deferred-panel" role="status"><LoaderCircle className="spin" size={18}/><span>{label}</span></div>;
}
...
export function DocumentBuilderClient({ initialUser, signInPath, initialDocumentId, printMode = false, initialConsultation = null }: {
  ...
  return <div className="dbt-root"><BuilderHeader user={user} signInPath={signInPath}/><div className="dbt-builder">
    ...
    <div className="dbt-workspace"><div className="dbt-form-column"><BuilderQuestionnaire answers={answers} onChange={changeAnswers} step={step} profile={profile} contacts={contacts} onSaveProfile={saveProfile} onUpdateContact={updateContact}/><div className="dbt-form-nav"><button type="button" onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0}><ArrowLeft size={17}/>Назад</button>{step < 4 ? <button type="button" className="primary" onClick={() => setStep(Math.min(4, step + 1))}>Продолжить<ArrowRight size={17}/></button> : <button type="button" className="primary" onClick={() => void generate()} disabled={generating}>{generating ? <><LoaderCircle className="spin" size={18}/>Формируем DOCX, PDF и ZIP…</> : user ? <>Создать файлы<FileCheck2 size={18}/></> : <>Войти и создать файлы<LockKeyhole size={18}/></>}</button>}</div>{user && step === 4 && <Suspense fallback={<DeferredPanelFallback label="Загружаем анализ…"/>}><DeferredBuilderAnalysisLauncher locale={answers.language === "ru" ? "ru" : "uz"} reviewPath={paths.documentReview} onPrepare={async () => { const id = await createDraft(); await persist(id); return id; }}/></Suspense>}</div><DocumentPreview document={visibleReceipt} mobileOpen={mobilePreview} onClose={() => setMobilePreview(false)}/></div>
    <div className="dbt-editor-wrap"><div className="dbt-editor-heading"><button type="button" onClick={() => setEditorOpen(!editorOpen)}><PenLine size={17}/>{editorOpen ? "Скрыть ручной редактор" : "Открыть ручной редактор"}</button>{!user && <span>Доступен после входа</span>}</div>{editorOpen && <Suspense fallback={<DeferredPanelFallback label="Открываем редактор…"/>}><DeferredManualEditor value={finalText || autoText} onChange={editFinalText} onUndo={undo} onRedo={redo} onReset={resetText} canUndo={undoStack.length > 0} canRedo={redoStack.length > 0} locked={!user}/></Suspense>}</div>
    {documentId && user && <><Suspense fallback={<DeferredPanelFallback label="Загружаем историю версий…"/>}><DeferredBuilderVersionHistory documentId={documentId} locale={answers.language === "ru" ? "ru" : "uz"} refreshKey={revision} onPrepare={async () => { await persist(documentId); return { documentId, revision: revisionRef.current }; }} onRestored={async () => { const result = await apiFetch<{ document: StoredDocument }>(`/api/document-builder/documents/${documentId}`); skipNextAutosave.current = true; hydrateDocument(result.document); }}/></Suspense><Suspense fallback={<DeferredPanelFallback label="Загружаем файлы документа…"/>}><DeferredDocumentAssetsPanel documentId={documentId} onDocumentChange={syncDocumentMetadata}/></Suspense><Suspense fallback={<DeferredPanelFallback label="Загружаем совместную работу…"/>}><DeferredCollaborationPanel documentId={documentId} accessRole="owner" finalText={finalText || autoText} currentUserEmail={user.email} signedFileId={signedFileId} onApplied={() => window.location.reload()}/></Suspense></>}
  </div></div>;
}
