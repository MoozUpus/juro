...
type Folder = "all" | "created" | "favorite" | "archive";
...
export function DocumentsClient({ user, signInPath }: { user: ChatGPTUser; signInPath: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = await apiFetch<{ documents: Array<DocumentRecord & { accessRole: "owner" | "collaborator" }>; standaloneFiles: FileRecord[]; total: number }>(`/api/document-builder-test/documents?${params}`);
  ...
  const patch = async (id: string, body: Record<string, unknown>) => {
    await apiFetch(`/api/document-builder-test/documents/${id}`, { method: "PATCH", body: JSON.stringify(body) }); setMenu(""); await load();
  };
  const duplicate = async (id: string) => {
    const result = await apiFetch<{ document: DocumentRecord }>("/api/document-builder-test/documents", { method: "POST", body: JSON.stringify({ sourceDocumentId: id }) });
    window.location.assign(`/document-builder-test/documents/${result.document.id}`);
  };
  const removeDocument = async (document: DocumentRecord, policy?: "keep" | "delete") => {
    try {
      const query = policy ? `?signed=${policy}` : "";
      await apiFetch(`/api/document-builder-test/documents/${document.id}${query}`, { method: "DELETE" });
  ...
  const uploadSigned = async (document: DocumentRecord, file: File) => {
    ...
    try { await apiFetch(`/api/document-builder-test/documents/${document.id}/signed-file`, { method: "POST", body: form }); await load(); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "PDF не загружен."); }
  };
  const patchStandalone = async (id: string, body: Record<string, unknown>) => { await apiFetch(`/api/document-builder-test/standalone-files/${id}`, { method: "PATCH", body: JSON.stringify(body) }); await load(); };
  const deleteStandalone = async (file: FileRecord) => { await apiFetch(`/api/document-builder-test/standalone-files/${file.id}`, { method: "DELETE" }); await load(); };
  const loadStandaloneShare = async (id: string) => {
    const result = await apiFetch<{ share: StandaloneShare | null }>(`/api/document-builder-test/standalone-files/${id}/share`); setStandaloneShare((value) => ({ ...value, [id]: result.share }));
  };
  const createStandaloneShare = async (id: string) => {
    try {
      const result = await apiFetch<{ share: StandaloneShare }>(`/api/document-builder-test/standalone-files/${id}/share`, { method: "POST", body: JSON.stringify({ action: "create" }) });
  ...
  const deleteExpiredShare = async (id: string) => {
    await apiFetch(`/api/document-builder-test/standalone-files/${id}/share`, { method: "POST", body: JSON.stringify({ action: "delete_expired" }) }); setStandaloneShare((value) => ({ ...value, [id]: null }));
  };
  ...
  return <div className="dbt-root"><BuilderHeader user={user} signInPath={signInPath}/><main className="dbt-documents-page">
    <header className="dbt-page-title"><div><span><Files size={22}/></span><div><h1>Мои документы</h1><p>{total} {total === 1 ? "документ" : "документов и файлов"}</p></div></div><a href="/document-builder-test"><FilePlus2 size={18}/>Создать документ</a></header>
