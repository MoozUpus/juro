...
const copy = {
  ru: { title: "Состояние источников", refresh: "Обновить статус", fresh: "База источников свежая", stale: "База источников требует проверки", unknown: "Нет успешной синхронизации", review: "На legal review", fetch: "В очереди загрузки", latest: "Последняя успешная синхронизация", unavailable: "Не удалось получить защищённый статус.", lex: "lex.uz", advice: "advice.uz", runs: "Последние запуски", never: "нет данных" },
  uz: { title: "Manbalar holati", refresh: "Holatni yangilash", fresh: "Manbalar bazasi dolzarb", stale: "Manbalar bazasi tekshiruvni talab qiladi", unknown: "Muvaffaqiyatli sinxronlash yo‘q", review: "Legal review’da", fetch: "Yuklash navbatida", latest: "Oxirgi muvaffaqiyatli sinxronlash", unavailable: "Himoyalangan holatni olish imkoni bo‘lmadi.", lex: "lex.uz", advice: "advice.uz", runs: "Oxirgi ishga tushirishlar", never: "ma’lumot yo‘q" },
} as const;
...
export function LegalSourceHealthPanel({ locale }: { locale: Locale }) {
  ...
  return <section className="staff-health" aria-labelledby="source-health-title" aria-busy={busy}>
    ...
    {error ? <p className="staff-health-error" role="status">{error}</p> : <div className="staff-health-grid">
      ...
      <div><span>{l.review}</span><b>{health?.pendingReviewCount ?? "—"}</b></div><div><span>{l.fetch}</span><b>{health?.pendingFetchCount ?? "—"}</b></div>
