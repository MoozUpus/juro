...
const copy = {
  ru: {
    ...
    cancel: "Отмена", success: "Решение сохранено. Публичная проекция обновляется только для одобренного ответа.", error: "Не удалось выполнить запрос.", protected: "Защищённый контур · свежая 2FA",
  },
  uz: {
    ...
    cancel: "Bekor qilish", success: "Qaror saqlandi. Ommaviy ko‘rinish faqat tasdiqlangan javob uchun yangilanadi.", error: "So‘rov bajarilmadi.", protected: "Himoyalangan kontur · yangi 2FA",
  },
} as const;
...
export function LawyerReviewReplyModerationInbox({ locale, reviewerName }: { locale: Locale; reviewerName: string }) {
  const t = copy[locale];
  ...
  const load = useCallback(async () => {
    ...
    try {
      const payload = await json<{ replies: Reply[] }>(await fetch(`/api/platform/admin/lawyer-review-replies?status=${encodeURIComponent(status)}`, { cache: "no-store" }));
