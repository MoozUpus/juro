    ...
    <main className="staff-main" id="main-content">
      ...
      >
        ...
        <div className="cost-table-wrap">
          <table>
            <caption className="sr-only">{t.priceVerification}</caption>
            <thead><tr><th scope="col">{t.model}</th><th scope="col">{t.expectedRate}</th><th scope="col">{t.activeRate}</th><th scope="col">{t.verificationStatus}</th></tr></thead>
      ...
      <section className="cost-usage" aria-labelledby="cost-plan-usage-title">
        ...
        {data.byPlan.length ? <div className="cost-table-wrap"><table><caption className="sr-only">{t.planUsage}</caption><thead><tr><th scope="col">{t.currentPlan}</th><th scope="col">{t.users}</th><th scope="col">{t.requests}</th><th scope="col">{t.failures}</th><th scope="col">{t.cost}</th></tr></thead><tbody>{data.byPlan.map((row) => <tr key={`${row.attribution}:${row.planCode ?? "none"}`}><td>{row.attribution === "guest_or_system" ? t.guestOrSystem : row.attribution === "unassigned" ? t.unassignedPlan : <code>{row.planCode}</code>}</td><td>{row.userCount}</td><td>{row.requestCount}</td><td>{row.failedRequestCount}</td><td>{usd(row.estimatedCostMicrousd)}{row.unpricedRequestCount > 0 ? ` · ${t.unpriced}: ${row.unpricedRequestCount}` : ""}</td></tr>)}</tbody></table></div> : <p className="staff-empty">{t.noData}</p>}
      </section>

      <section className="cost-usage" aria-labelledby="cost-user-usage-title">
        <h2 id="cost-user-usage-title">{t.userUsage}</h2>
        {data.byUser.length ? <div className="cost-table-wrap"><table><caption className="sr-only">{t.userUsage}</caption><thead><tr><th scope="col">{t.user}</th><th scope="col">{t.workspace}</th><th scope="col">{t.currentPlan}</th><th scope="col">{t.requests}</th><th scope="col">{t.failures}</th><th scope="col">{t.tokens}</th><th scope="col">{t.cost}</th></tr></thead><tbody>{data.byUser.map((row) => <tr key={`${row.workspaceId}:${row.userId}`}><td><code title={row.userId}>{shortId(row.userId)}</code></td><td><code title={row.workspaceId}>{shortId(row.workspaceId)}</code></td><td>{row.currentPlanCode ? <code>{row.currentPlanCode}</code> : t.unassignedPlan}</td><td>{row.requestCount}</td><td>{row.failedRequestCount}</td><td>{row.inputTokens + row.outputTokens}</td><td>{usd(row.estimatedCostMicrousd)}{row.unpricedRequestCount > 0 ? ` · ${t.unpriced}: ${row.unpricedRequestCount}` : ""}</td></tr>)}</tbody></table></div> : <p className="staff-empty">{t.noData}</p>}
      </section>

      <section className="cost-usage" aria-labelledby="cost-alerts-title">
        <h2 id="cost-alerts-title">{t.alerts}</h2>
        {data.alerts.length || data.scopeBudgetAlerts.length ? <div className="cost-table-wrap"><table><caption className="sr-only">{t.alerts}</caption><thead><tr><th scope="col">{t.date}</th><th scope="col">{t.provider}</th><th scope="col">{t.reason}</th><th scope="col">{t.status}</th></tr></thead><tbody>{data.alerts.map((alert) => <tr key={alert.id}><td><time dateTime={alert.createdAt}>{new Date(alert.createdAt).toLocaleString(locale === "ru" ? "ru-RU" : "uz-UZ")}</time></td><td>{alert.provider}</td><td>{alert.reason}</td><td>{alert.status}{alert.errorCode ? ` · ${alert.errorCode}` : ""}</td></tr>)}{data.scopeBudgetAlerts.map((alert) => <tr key={alert.id}><td><time dateTime={alert.createdAt}>{new Date(alert.createdAt).toLocaleString(locale === "ru" ? "ru-RU" : "uz-UZ")}</time></td><td><code title={alert.scopeKey}>{alert.scopeType === "user" ? shortId(alert.scopeKey) : alert.scopeKey}</code></td><td>{alert.periodType} · {alert.reason}</td><td>{alert.status}{alert.errorCode ? ` · ${alert.errorCode}` : ""}</td></tr>)}</tbody></table></div> : <p className="staff-empty">{t.noData}</p>}
      </section>
    </main>
  </div>;
}
