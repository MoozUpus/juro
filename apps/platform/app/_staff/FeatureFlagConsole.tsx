...
const labels: Record<OperationalLocale, Record<OperationalFeatureKey, string>> = {
  ru: { ai_chat: "AI-чат", ai_openai_primary: "OpenAI — основной", ai_anthropic_fallback: "Anthropic — резерв", ai_lex_web_discovery: "OpenAI-поиск Lex.uz", ai_secondary_web_research: "Дополнительный веб-поиск", document_analysis_upload: "Загрузка для анализа", lawyer_handoff: "Передача юристу", voice_mode: "Голосовой режим" },
  uz: { ai_chat: "AI-chat", ai_openai_primary: "OpenAI — asosiy", ai_anthropic_fallback: "Anthropic — zaxira", ai_lex_web_discovery: "OpenAI orqali Lex.uz qidiruvi", ai_secondary_web_research: "Qo‘shimcha veb-qidiruv", document_analysis_upload: "Tahlil uchun yuklash", lawyer_handoff: "Yuristga topshirish", voice_mode: "Ovozli rejim" },
  en: { ai_chat: "AI chat", ai_openai_primary: "OpenAI — primary", ai_anthropic_fallback: "Anthropic — fallback", ai_lex_web_discovery: "OpenAI Lex.uz discovery", ai_secondary_web_research: "Secondary web research", document_analysis_upload: "Document analysis uploads", lawyer_handoff: "Lawyer handoff", voice_mode: "Voice mode" },
};

const copy = {
  ru: { skip: "К основному содержимому", title: "Аварийное управление функциями", secure: "Защищённая рабочая зона", fresh: "Недавняя 2FA", environment: "Среда", refresh: "Обновить", enabled: "Работает", disabled: "Приостановлено", disable: "Приостановить", enable: "Возобновить", reason: "Причина изменения", reasonHint: "Не менее 10 символов. Не указывайте секреты или пользовательские данные.", save: "Записать изменение", saved: "Изменение записано в неизменяемую историю", integrityOk: "Цепочка истории подтверждена", integrityBad: "Нарушена целостность истории. Изменения заблокированы.", history: "История изменений", noHistory: "Изменений пока нет. Все функции используют безопасное состояние по умолчанию.", version: "Версия", actor: "Оператор", cancel: "Отмена" },
  ...
  en: { skip: "Skip to main content", title: "Emergency feature controls", secure: "Secure workspace", fresh: "Recent 2FA", environment: "Environment", refresh: "Refresh", enabled: "Operational", disabled: "Paused", disable: "Pause", enable: "Resume", reason: "Reason for change", reasonHint: "Use at least 10 characters. Do not include secrets or user data.", save: "Record change", saved: "The change was recorded in immutable history", integrityOk: "The history chain is verified", integrityBad: "History integrity verification failed. Changes are blocked.", history: "Change history", noHistory: "No changes yet. All features are using their safe default state.", version: "Version", actor: "Operator", cancel: "Cancel" },
} as const;
...
export function FeatureFlagConsole({ locale, staffName, initial }: { locale: OperationalLocale; staffName: string; initial: Dashboard }) {
  ...
  return <div className="staff-console" aria-busy={busy}>
    ...
    <header className="staff-topbar"><div className="staff-brand"><ShieldCheck aria-hidden="true"/><span><b>JURO</b><small>{t.secure}</small></span></div><div className="staff-session"><span>{t.fresh}</span><b>{staffName}</b></div><a href={`/${nextLocale}/admin/feature-flags`} hrefLang={nextLocale}>{nextLocale.toUpperCase()}</a></header>
    <main id="feature-flags-main" className="staff-main feature-flags-main">
      ...
      {selected ? <form className="staff-decision feature-flag-form" onSubmit={(event) => void submit(event)}>
        ...
        <div><button type="button" className="staff-reject" onClick={() => setSelectedKey(null)} disabled={busy}>{t.cancel}</button><button className="staff-approve" disabled={busy || !dashboard.integrity.valid}>{t.save}</button></div>
      </form> : null}
