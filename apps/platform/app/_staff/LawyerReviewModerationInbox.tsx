...
export function LawyerReviewModerationInbox({ locale, reviewerName }: { locale: Locale; reviewerName: string }) {
  ...
  return <div className="staff-console">
    ...
    <main id="staff-main" className="staff-main">
      <section className="staff-heading"><div><span>JURO · REVIEW SAFETY</span><h1>{t.title}</h1><p>{t.description}</p></div><button type="button" onClick={() => void load()} disabled={busy}><RefreshCw aria-hidden="true"/>{t.refresh}</button></section>
