  ...
  return <div className="staff-console audit-console" aria-busy={busy}>
    ...
    <main id="audit-main" className="staff-main audit-main">
      ...
      <section className="audit-table-wrap" aria-label={t.title}>
        {busy && !rows.length ? <p className="staff-empty">{t.loading}</p> : rows.length ? <table><caption className="sr-only">{t.title}</caption><thead><tr><th scope="col">{t.created}</th><th scope="col">{t.source}</th><th scope="col">{t.severity}</th><th scope="col">{t.action}</th><th scope="col">{t.entity}</th><th scope="col">{t.identifiers}</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.source}:${row.id}`}><td><time dateTime={row.createdAt}>{timestamp(row.createdAt)}</time></td><td>{row.source}</td><td><span className={`audit-severity audit-severity--${row.severity}`}>{row.severity}</span></td><td><code>{row.action}</code></td><td><b>{row.entityType}</b><small>{row.entityId ?? "—"}</small></td><td><details><summary>{t.identifiers}</summary><dl><div><dt>ID</dt><dd>{row.id}</dd></div><div><dt>actor</dt><dd>{row.actorUserId ?? "system"}</dd></div><div><dt>scope</dt><dd>{row.scopeId}</dd></div></dl></details></td></tr>)}</tbody></table> : <p className="staff-empty">{t.empty}</p>}
      </section>
    </main>
  </div>;
}
