...
export default async function PrivacySettingsPage({ params }: { params: Promise<{ locale: string; accountType: string }> }) {
  const { locale, accountType } = await params;
  if (!isLocale(locale) || !isAccountType(accountType)) notFound();
  return <ProfileSettingsClient locale={locale} accountType={accountType} view="privacy" />;
}
