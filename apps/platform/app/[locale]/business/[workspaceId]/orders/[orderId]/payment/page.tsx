...
export default async function BusinessOrderPaymentPage({ params }: { params: Promise<{ locale: string; workspaceId: string; orderId: string }> }) {
  const { locale, workspaceId, orderId } = await params;
  if (!isLocale(locale) || !isWorkspaceId(workspaceId) || !orderId) notFound();
  await requireChatGPTUser(`/${locale}/business/${encodeURIComponent(workspaceId)}/orders/${encodeURIComponent(orderId)}/payment`);
