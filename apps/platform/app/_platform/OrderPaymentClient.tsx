...
type PaymentView = {
  ...
  order: { id: string; status: string; totalAmountMinor: number; providerStatus: string | null; settledAt: string | null };
...
export function OrderPaymentClient({ locale, accountType, orderId, workspaceId }: { locale: PlatformLocale; accountType: AccountType; orderId: string; workspaceId?: string }) {
  ...
  const active = view.order.status === "ACTIVE" && view.invoice?.status === "paid";
  ...
  return <section className="checkout-workspace payment-status-workspace">
    ...
    <header className={active ? "payment-success" : ""}><div><small>JURO · PAYMENT</small><h1>{active ? (ru ? "Оплата подтверждена" : "To‘lov tasdiqlandi") : failed ? (ru ? "Оплата отклонена" : "To‘lov rad etildi") : (ru ? "Подтверждение оплаты" : "To‘lovni tasdiqlash")}</h1><p>{active ? (ru ? "Подписка активирована только после проверенного серверного события. Платёж и проводки сохранены." : "Obuna faqat tekshirilgan server hodisasidan keyin faollashtirildi. To‘lov va yozuvlar saqlandi.") : (ru ? "В staging можно проверить успешный и отклонённый сценарии без списания реальных денег." : "Stagingda haqiqiy pul yechmasdan muvaffaqiyatli va rad etilgan ssenariylarni tekshirish mumkin.")}</p></div>{active ? <CheckCircle2/> : <ShieldCheck/>}</header>
