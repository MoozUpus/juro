...
export function LawyerDirectoryClient({ locale, accountType, workspaceId }: { locale: PlatformLocale; accountType: AccountType; workspaceId?: string }) {
  ...
  return <section className="lawyer-directory" aria-labelledby="lawyer-directory-heading">
    <header className="lawyer-directory-hero">
      ...
      <div><span>JURO · MARKETPLACE</span><h1 id="lawyer-directory-heading">{ru ? "Юристы и адвокаты" : "Yuristlar va advokatlar"}</h1><p>{ru ? "В каталоге отображаются только профили, отдельно одобренные JURO. Передача материалов дела всегда требует вашего подтверждения." : "Katalogda faqat JURO alohida tasdiqlagan profillar ko‘rsatiladi. Ish materiallarini topshirish doimo sizning tasdig‘ingizni talab qiladi."}</p></div>
    </header>
    ...
    <div className="lawyer-directory-grid">
      {results.map((lawyer) => <article key={lawyer.id} className="lawyer-directory-card">
        <div className="lawyer-directory-card-head"><div><h2>{lawyer.displayName}</h2><p>{lawyer.firmName || (ru ? "Независимый специалист" : "Mustaqil mutaxassis")}</p></div>{lawyer.advocateStatus === "verified" && <span className="lawyer-verified"><ShieldCheck aria-hidden="true" />{ru ? "Проверено JURO" : "JURO tasdiqlagan"}</span>}</div>
