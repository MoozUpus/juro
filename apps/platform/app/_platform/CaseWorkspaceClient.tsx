...
function CaseSectionPanel({ section, locale, base, item, tasks, data }: { section: CaseSection; locale: PlatformLocale; base: string; item: CaseRecord; tasks: Task[]; data: CaseWorkspaceData }) {
  ...
  if (section === "analyses") return <SimpleList icon={FileSearch2} title={ru ? "Анализы и сравнения" : "Tahlil va taqqoslashlar"} empty={ru ? "К делу ещё не привязано сравнение документов. Самостоятельные анализы остаются в разделе анализа до явной привязки." : "Ishga hali hujjat taqqoslash biriktirilmagan. Mustaqil tahlillar aniq biriktirilmaguncha tahlil bo‘limida qoladi."} items={(data.comparisons ?? []).map((comparison) => ({ id: comparison.id, title: ru ? "Сравнение версий документа" : "Hujjat nusxalarini taqqoslash", meta: `${statusLabel(comparison.status, ru)}${comparison.overallRisk ? ` · ${riskLabel(comparison.overallRisk, ru)}` : ""} · ${date(comparison.updatedAt, locale)}`, href: `${base}/documents/comparisons/${encodeURIComponent(comparison.id)}` }))} action={<Link href={`${base}/document-review?caseId=${encodeURIComponent(item.id)}`}>{ru ? "Анализировать документ" : "Hujjatni tahlil qilish"}<ArrowUpRight /></Link>} />;
