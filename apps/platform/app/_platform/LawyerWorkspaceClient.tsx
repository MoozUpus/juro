...
function LawyerRecordsClient({ locale, view }: { locale: PlatformLocale; view: string }) {
  ...
  return <section className="lawyer-workspace lawyer-records"><header className="lawyer-records-header"><Icon /><div><small>JURO · {ru ? "кабинет юриста" : "yurist kabineti"}</small><h1>{definition.title}</h1><p>{definition.description}</p></div></header>{error && <p className="lawyer-workspace-error" role="alert">{error}</p>}{loading && !data ? <div className="lawyer-workspace-loading"><LoaderCircle className="spin" /></div> : <div className="lawyer-record-list">
    ...
    {view === "documents" && data?.ownDocuments.map((item) => <Link href={`${base}/documents/${encodeURIComponent(item.id)}`} key={`own-${item.id}`}><FileText /><div><strong>{item.title}</strong><small>{ru ? "Ваш проект" : "Sizning loyihangiz"} · {documentStatusLabel(item.status, locale)}</small></div><time>{formatDate(item.updatedAt, ru)}</time></Link>)}
    {view === "documents" && data?.documents.map((item) => <Link href={`${base}/documents/${encodeURIComponent(item.id)}`} key={item.id}><FileText /><div><strong>{item.title}</strong><small>{documentCategoryLabel(item.category, locale)} · {documentStatusLabel(item.status, locale)}</small></div><time>{formatDate(item.updatedAt, ru)}</time></Link>)}
