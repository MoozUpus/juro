...
export function HistoryClient({ locale }: { locale: PlatformLocale }) {
  ...
  return <section className="history-workspace"><header><History /><div><small>{ru ? "JURO · ХРОНОЛОГИЯ" : "JURO · XRONOLOGIYA"}</small><h1>{ru ? "История действий" : "Harakatlar tarixi"}</h1><p>{ru ? "Проверяемая хронология работы: вопросы, документы, дела и системные проверки." : "Tekshiriladigan ish xronologiyasi: savollar, hujjatlar, ishlar va tizim tekshiruvlari."}</p></div></header>{error && <p className="history-error"><CircleAlert />{error}</p>}<div className="history-filter"><Filter /><label><span className="sr-only">{ru ? "Фильтр истории" : "Tarix filtri"}</span><select value={filter} onChange={event => setFilter(event.target.value)}><option value="all">{ru ? "Все события" : "Barcha voqealar"}</option><option value="workspace">{ru ? "Пространство" : "Makon"}</option><option value="document">{ru ? "Документы" : "Hujjatlar"}</option><option value="case">{ru ? "Дела" : "Ishlar"}</option></select></label></div>{loading ? <div className="history-loading"><LoaderCircle className="spin" /></div> : visible.length ? <ol className="history-list">{visible.map(event => <li key={`${event.source}-${event.id}`}><span>{sourceLabel(event.source, ru)}</span><div><strong>{actionLabel(event.action, ru)}</strong><small>{entityLabel(event.entityType, ru)}</small></div><time>{formatDateTime(event.createdAt, ru)}</time></li>)}</ol> : <div className="history-empty"><History /><h2>{ru ? "Событий пока нет" : "Hozircha voqealar yo‘q"}</h2></div>}</section>;
}
...
function actionLabel(action: string, ru: boolean) {
  const labels: Record<string, [string, string]> = {
    ...
    ai_chat_completed: ["AI-ответ подготовлен", "AI javobi tayyorlandi"],
    upload_initiated: ["Загрузка файла начата", "Fayl yuklash boshlandi"],
    upload_completed: ["Файл загружен", "Fayl yuklandi"],
    upload_quarantined: ["Файл изолирован для проверки", "Fayl tekshiruv uchun ajratildi"],
    malware_scan_clean: ["Проверка файла завершена", "Fayl tekshiruvi yakunlandi"],
    invitation_sent: ["Отправлено приглашение", "Taklif yuborildi"], invitation_accepted: ["Приглашение принято", "Taklif qabul qilindi"],
  };
  return labels[action]?.[ru ? 0 : 1] ?? (ru ? "Системное действие" : "Tizim harakati");
}
function entityLabel(entityType: string, ru: boolean) {
  const labels: Record<string, [string, string]> = {
    conversation: ["Юридический диалог", "Yuridik suhbat"],
    document: ["Документ", "Hujjat"],
    document_analysis: ["Анализ документа", "Hujjat tahlili"],
    case: ["Юридическое дело", "Yuridik ish"],
    workspace: ["Рабочее пространство", "Ish makoni"],
    user: ["Профиль", "Profil"],
  };
  return labels[entityType]?.[ru ? 0 : 1] ?? (ru ? "Событие пространства" : "Makon voqeasi");
}
