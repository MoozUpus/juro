...
export function LawyerHandoffClient({ locale, accountType, workspaceId }: { locale: PlatformLocale; accountType: AccountType; workspaceId?: string }) {
  ...
  return <section className="lawyer-handoff" aria-labelledby="lawyer-handoff-heading">
    ...
    {requests.length > 0 && <div className="lawyer-handoff-list"><h3>{ru ? "Мои заявки к юристу" : "Yuristga yuborgan so‘rovlarim"}</h3>{requests.map((item) => <div key={item.id}><strong>{handoffStatus(item.status, ru)}</strong><span>{item.lawyerName || (ru ? "Ожидается назначение JURO" : "JURO tayinlashi kutilmoqda")}</span><time dateTime={item.createdAt}>{new Intl.DateTimeFormat(ru ? "ru-RU" : "uz-UZ", { dateStyle: "medium", timeZone: "Asia/Tashkent" }).format(new Date(item.createdAt))}</time>{item.status === "awaiting_user_consent" && <div className="lawyer-access-action"><label className="consult-consent"><input type="checkbox" checked={Boolean(accessConsents[item.id])} disabled={accessActionId === item.id} onChange={(event) => setAccessConsents((current) => ({ ...current, [item.id]: event.target.checked }))} /><span>{ru ? "Подтверждаю передачу выбранному юристу материалов этого дела и взаимное раскрытие наших номеров телефона для обычного звонка. Доступ можно отозвать в любой момент." : "Tanlangan yuristga ushbu ish materiallarini berish va oddiy qo‘ng‘iroq uchun telefon raqamlarimizni o‘zaro ko‘rsatishni tasdiqlayman. Ruxsatni istalgan paytda bekor qilish mumkin."}</span></label><button type="button" disabled={!accessConsents[item.id] || accessActionId === item.id} onClick={() => void updateCaseAccess(item, "grant")}>{accessActionId === item.id ? <LoaderCircle className="spin" /> : null}{ru ? "Предоставить доступ" : "Ruxsat berish"}</button></div>}{item.activeGrantId && <div className="lawyer-access-action"><p>{ru ? "У юриста есть доступ к материалам этого дела." : "Yurist ushbu ish materiallariga ruxsatga ega."}</p><ClientServiceProposals locale={locale} accountType={accountType} workspaceId={workspaceId} caseId={item.caseId} />{item.offerId && <div className="lawyer-offer-card"><strong>{offerLabel(item.offerStatus, ru)}</strong><p>{item.offerScopeDescription}</p><p>{ru ? "Стоимость: " : "Narx: "}{item.offerPriceDescription}</p><p>{ru ? "Срок: " : "Muddat: "}{item.offerDurationDescription}</p>{item.offerStatus === "proposed" && <div className="lawyer-offer-actions"><button type="button" disabled={accessActionId === item.id} onClick={() => void respondToOffer(item, "accepted")}>{ru ? "Принять внешние условия" : "Tashqi shartlarni qabul qilish"}</button><button type="button" className="secondary" disabled={accessActionId === item.id} onClick={() => void respondToOffer(item, "declined")}>{ru ? "Отклонить" : "Rad etish"}</button></div>}</div>}<LawyerPhoneContact requestId={item.id} locale={locale} /><button type="button" className="secondary" disabled={accessActionId === item.id} onClick={() => void updateCaseAccess(item, "revoke")}>{accessActionId === item.id ? <LoaderCircle className="spin" /> : null}{ru ? "Отозвать доступ" : "Ruxsatni bekor qilish"}</button></div>}{item.activeGrantId && <LawyerRequestMessages requestId={item.id} locale={locale} />}{item.status === "completed" && <LawyerReviewForm requestId={item.id} locale={locale} />}</div>)}</div>}
  </section>;
}
