...
CREATE TRIGGER `legal_custom_search_membership_lookups_no_update`
BEFORE UPDATE ON `legal_custom_search_membership_lookups`
BEGIN SELECT RAISE(ABORT, 'LEGAL_CUSTOM_MEMBERSHIP_LOOKUP_IMMUTABLE'); END;
--> statement-breakpoint
CREATE TRIGGER `legal_custom_search_membership_lookups_no_delete`
BEFORE DELETE ON `legal_custom_search_membership_lookups`
BEGIN SELECT RAISE(ABORT, 'LEGAL_CUSTOM_MEMBERSHIP_LOOKUP_IMMUTABLE'); END;
