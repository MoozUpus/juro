...
| Session persistence and MFA rotation | 24-hour default and 30-day explicit remember-me absolute lifetimes; aligned cookie `Max-Age` and D1 expiry; same choice after OTP or MFA; strict boolean inputs with false default; existing seven-day idle cap; MFA elevation and MFA disable each atomically retire the current token digest, return a replacement cookie, and preserve absolute expiry; one stale-token replay claim revokes the affected session/device and records a critical audit event; intervening-token/concurrent-disable races leave one guarded winner and no partial side effects; staging migration `0029`, restore drill, and the elevation Worker deployment pass | Deploy and exercise MFA-disable rotation in protected staging; exact HTTP/cookie/replay behavior for both triggers; periodic and email-change rotation; region signals and security email |
