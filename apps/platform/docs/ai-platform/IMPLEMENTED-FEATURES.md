...
| Session persistence and MFA rotation | 24-hour default and 30-day explicit remember-me absolute lifetimes; aligned cookie `Max-Age` and D1 expiry; same choice after OTP or MFA; strict boolean inputs with false default; existing seven-day idle cap; MFA elevation, MFA disable, and confirmed email change each atomically retire the current token digest, return a replacement cookie, and preserve absolute expiry; one stale-token replay claim revokes the affected session/device and records a critical audit event; intervening-token/concurrent-disable/concurrent-email-change races leave one guarded winner and no partial side effects; staging migration `0029`, restore drill, and the elevation Worker deployment pass | Deploy and exercise email-change rotation; exact authenticated HTTP/cookie/replay behavior for all three triggers; periodic rotation and region signals |
...
- 252 core/auth/document tests;
- 72 Cloudflare/migration/job tests, including migrations `0029`–`0030`, encrypted security-email evidence, and queue concurrency/replay contracts;
- 351 tests total.
