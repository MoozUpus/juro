...
| Integration branch | SEC-001 and SEC-002 have local fixes; SEC-003 has atomic verification plus the local `0023` lock; SEC-005 has local independent rate controls and Turnstile integration; structured onboarding is additive in `0024`; migrations `0025`–`0027` add a local fail-closed legal-source lifecycle from exact official URL and private R2 through deterministic normalization and MFA-bound immutable legal-review evidence; source migrations `0022`–`0027` are not in staging and SEC-004 remains open | no upload/AI staging enablement; identity and legal-source changes still require remote migrations, protected review/publisher HTTP boundaries, live-provider/network, runtime, R2, and full-HTTP staging evidence; Advice remains disabled |
