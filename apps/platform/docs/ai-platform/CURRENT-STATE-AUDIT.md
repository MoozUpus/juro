...
| GitHub draft PR #3, `feature/juro-ai-platform` | Pushed code checkpoint `0544a5668e982762921da78658571b5a010fb48d`; the staging-evidence documentation accompanies the next branch checkpoint | The draft PR tracks the same feature branch; source migration `0029` and the session-rotation defense are pushed without rewriting shared history. Isolated staging is through `0029`; current commit/push state must be read from Git rather than inferred from this audit snapshot. |

The original comparison found 116 files present only in the Sites source, 50 materially changed files, one GitHub-only file, and 179 identical files. The verified Sites source and Phase 0 audit were synchronized into the local `feature/juro-ai-platform` lineage in commit `c454d779e1ec91c6a1a1ad270c9d1b7b02afabb7`. On 2026-07-28 the five later `main` commits through `a1c572e` were merged in `702960e` without rebasing or discarding the existing work. Draft PR #3 contains every committed code checkpoint through `0544a56`; this document records the subsequent staging evidence without attempting to predict its final commit SHA.
...
- D1 `juro-staging` — `bb716a96-b2fb-4823-90d6-6c228fed181a`, EEUR, exact 30-entry `0000`–`0029` ledger, 109 non-internal tables, and 58 triggers;
...
- eight legacy development-only queues remain unbound; 28 development/staging v2 primary + DLQ queues exist with 86,400-second retention; the seven staging primary queues have producer bindings and no consumer;
- eight empty development/staging Vectorize v2 indexes exist at 1,536 dimensions/cosine;
- protected staging Worker `juro-platform-staging` serves deployment `888a4800-daf8-4211-b41d-a653d067ecd8`, version `448e5bf1-4bf8-4000-af2b-2c034e3eca10` at 100% from commit `288af4693d2679b48f016215caaabdcac9aa0fde`, with staging-only bindings and seven Queue producers. Subdomain/previews, schedules, and consumers are disabled; `staging.app.juro.uz` is attached behind owner-only Access; the public Turnstile binding and three server-only secret names are present. AI Gateway and Workers observability destinations remain unverified; production Queue/Vectorize and backup/quarantine resources remain absent.
...
Source reconciliation, control-plane inventory, threat/model audits, and the reproducible local baseline are complete locally; code checkpoint `288af4693d2679b48f016215caaabdcac9aa0fde` is pushed to draft PR #3 and passed exact-source CI run `30453980092`; this evidence set is intended for the same branch. Time Travel plus portable/private-R2/disposable-remote restore checkpoints protect staging through `0029`. Earlier authenticated Chrome evidence covers the canonical builder, RU/UZ route behavior, console, broken images, landmarks, and horizontal overflow at 320, 360, 390, 768, 1024, 1280, and 1440 px. The exact current Worker version has control-plane and anonymous Access evidence only: the available browser-control runtime now exits at startup because its generated CommonJS kernel is treated as ESM, so current-version authenticated UI/cookie/replay evidence remains open and Access was not bypassed.
...
The disabled Phase 1 foundation passes the inactive-deploy gates. The isolated staging D1 is through `0029`; ten verified D1 export/restore artifacts exist in private staging backup R2, 28 primary/DLQ queues and eight empty Vectorize indexes exist. Worker `juro-platform-staging` is deployed from commit `288af4693d2679b48f016215caaabdcac9aa0fde` as deployment `888a4800-daf8-4211-b41d-a653d067ecd8` / version `448e5bf1-4bf8-4000-af2b-2c034e3eca10` at 100%, with staging-only resource bindings and seven Queue producers; subdomain/previews, schedules, and consumers remain disabled, while the sole custom domain is protected by owner-only Access and anonymous denial is proved. The artifact sets `ASYNC_RUNTIME_ENABLED=false`, `LEGAL_ADVICE_INGESTION_ENABLED=false`, and `LEGAL_SOURCE_STAFF_API_ENABLED=false`; validation rejects `ALLOW_PLATFORM_AUTH_HEADERS`, consumers, and schedules. Activation remains blocked on current-version authenticated browser/HTTP evidence, live Turnstile/Resend, reviewed staff-route activation, consumer/DLQ policy, live Lex robots/fetch/R2 evidence, quarantine scanning, alerts, redrive, ledger reconciliation, per-kind flags, provider idempotency, and side-effect fencing. Advice additionally requires owner/legal approval. Sites v20 and legacy Worker `juro` remain unchanged.

## 2026-07-30 account-deletion release-candidate delta

The branch now contains a real account-deletion vertical slice rather than request-only evidence: immediate/recoverable modes, cancellation, blocker remediation/retry, dedicated purge execution across private R2 and D1, an irreversible fence, immutable profile tombstone, append-only lifecycle and purge evidence, scheduled outbox dispatch, and RU/UZ settings UI. Migration `0033` is still pending remotely in this checkpoint. Full local gates and a current private staging backup pass; production is unchanged.
