...
| GitHub draft PR #3, `feature/juro-ai-platform` | Pushed checkpoint `6fbd6d3b3026bcfd60b2b9856bd1d30dc1473091`; the dashboard/restore-evidence worktree is not yet committed | The draft PR tracks the same feature branch; this checkpoint reconciles Sites, source migrations through `0028`, security foundations, and GitHub `main` without rewriting shared history. Isolated staging is through `0028`; commit/push state must be read from Git rather than inferred from this audit snapshot. |

The original comparison found 116 files present only in the Sites source, 50 materially changed files, one GitHub-only file, and 179 identical files. The verified Sites source and Phase 0 audit were synchronized into the local `feature/juro-ai-platform` lineage in commit `c454d779e1ec91c6a1a1ad270c9d1b7b02afabb7`. On 2026-07-28 the five later `main` commits through `a1c572e` were merged in `702960e` without rebasing or discarding the existing work. Draft PR #3 now contains every committed checkpoint through `6fbd6d3`; the current dashboard/restore-evidence worktree remains uncommitted at this audit instant.
...
- D1 `juro-staging` — `bb716a96-b2fb-4823-90d6-6c228fed181a`, EEUR, exact 29-entry `0000`–`0028` ledger, 107 non-internal tables, and 58 triggers;
- private R2 `juro-private-documents`, legacy development stores, and six private EEUR Standard development/staging target buckets; only three verified D1 checkpoint exports exist in `juro-staging-backups`, and no binding was cut over;
...
- inactive staging Worker `juro-platform-staging` is deployed with staging-only bindings and seven Queue producers; subdomain/previews are disabled and routes/schedules/consumers/secrets/DNS remain absent. AI Gateway and Workers observability destinations remain unverified; production Queue/Vectorize and backup/quarantine resources remain absent.
...
- Local application of migrations `0000`–`0028` to an empty in-memory SQLite database succeeded.
- The resulting local database had 105 non-internal tables, 146 foreign keys, and zero foreign-key violations.
- The remote production and development D1 control plane reports 61 tables and migration ledgers `0000`–`0004`; migrations `0005`–`0028` are not applied to either database.
- The isolated staging D1 reports the exact ordered ledger `0000`–`0028`, 107 non-internal tables, and 58 triggers. The exact post-migration exported bytes restore locally with `integrity_check = ok` and zero foreign-key violations. A disposable remote EEUR D1 import then reproduced the same ledger/table/trigger inventory and zero foreign-key errors before the drill database was deleted. Remote D1 does not authorize the integrity pragma itself.
...
No production snapshot or migration was performed. On the initially empty EEUR staging database, the Time Travel restore/undo drill passed. Three later portable exports are retained in private `juro-staging-backups`; their bytes and SHA-256 values were verified, the post-`0028` artifact restored both locally and into a disposable remote EEUR D1, and the disposable D1 was deleted after schema/ledger/FK agreement. This establishes staging logical recoverability, not production protection or an operational RTO. D-040 permitted exactly one reproducible bootstrap of the originally verified-empty staging database: pre-bookmark `00000016-00000000-000050b6-d17b2ef8af450f78e2ba993d4272fe26` advanced to post-bookmark `00000016-00000036-000050b6-48eec1201b71eda52af14c1ba998f030`; the exception is consumed and is not a production or populated-database precedent.
...
Source reconciliation, control-plane inventory, threat/model audits, and the reproducible local baseline are complete locally; the current route/evidence worktree is not yet committed or pushed to draft PR #3. The empty-staging Time Travel drill passed; portable/private-R2/local and disposable-remote restore checkpoints then protected and verified staging through `0028`. The Browser runtime was recovered with a session-local temporary `{ "type": "commonjs" }` package scope; neither JURO nor the user-home package was changed. Authenticated Chrome evidence covers the canonical builder, RU/UZ route behavior, console, broken images, landmarks, and horizontal overflow at 320, 360, 390, 768, 1024, 1280, and 1440 px. A PII-free mobile screenshot is stored outside the repository in the task visualization workspace.

The complete Phase 0 gate is still open because a production routing decision and the remaining browser/accessibility matrix (200% zoom, keyboard/focus, reduced motion, axe, Lighthouse, real iOS/Android behavior, and broader critical-route screenshots) are not yet verified. The disposable remote-D1 logical import gate is complete, but no operational RTO is claimed. The Chrome client blocked the direct legacy `document-builder-test` navigation, so the browser attempt is not counted as redirect evidence; the existing HTTP/source regression evidence remains the recorded basis for that `308` contract.

The disabled Phase 1 foundation passes the inactive-deploy gates. The isolated staging D1 is through `0028`; three D1 exports exist in private staging backup R2, 28 primary/DLQ queues and eight empty Vectorize indexes exist. Worker `juro-platform-staging` is deployed from commit `29a3d9a` with staging-only resource bindings and seven Queue producers, while subdomain/previews, routes, DNS, schedules, consumers, and secrets remain absent. The artifact sets `ASYNC_RUNTIME_ENABLED=false`, `LEGAL_ADVICE_INGESTION_ENABLED=false`, and `LEGAL_SOURCE_STAFF_API_ENABLED=false`; validation rejects `ALLOW_PLATFORM_AUTH_HEADERS`, consumers, and schedules. A public staging hostname remains prohibited until access control is configured and unauthenticated denial is proved. Activation remains blocked on live Turnstile/Resend, reviewed staff-route activation plus browser evidence, consumer/DLQ policy, live Lex robots/fetch/R2 evidence, quarantine scanning, alerts, redrive, ledger reconciliation, per-kind flags, provider idempotency, and side-effect fencing. Advice additionally requires owner/legal approval. Sites v20 and legacy Worker `juro` were re-read unchanged.
