...
| GitHub draft PR #3, `feature/juro-ai-platform` | Pushed privileged-review checkpoint `d6567db`; the `0028` publisher slice follows locally until its commit/push gates pass | The draft PR tracks the same feature branch; this checkpoint reconciles Sites, source migrations through `0028`, security foundations, and GitHub `main` without rewriting shared history. The isolated staging schema remains at `0021`; commit/push state must be read from Git rather than inferred from this audit snapshot. |
...
- The isolated staging D1 reports the exact ordered ledger `0000`–`0021`, `PRAGMA quick_check = ok`, zero foreign-key violations, 98 tables including `d1_migrations`, and 275 schema objects. The seven migration-0011 control tables are present. Source migrations `0022`–`0028` have not been applied remotely.
...
| Legal knowledge | Disabled local acquisition, review, and first-version publication foundation only | migration `0025` adds fail-closed source/version/section/chunk/sync/review records; `0026` adds guarded single-page acquisition; normalization stores only an untrusted private parsed snapshot; `0027` adds dedicated-role/fresh-MFA immutable review decisions; `0028` adds a separate fresh-MFA publisher with R2/evidence revalidation, atomic verified-state transition, and immutable reading/publication evidence; consumers require exact verification evidence; Advice is policy-disabled and no successful live fetch, protected reviewer/publisher route/UI, replacement-version activation, hybrid retrieval, citation validator, indexing, Cron, or editor exists |
...
npm test                PASS
  ...
  core/auth/document tests: 234 PASS
...
The latest recorded successful local full suite is the 326-test run above: 25 rendered-route, 234 core, and 67 Cloudflare tests. It includes local coverage for migrations `0022`–`0028`, workspace-invitation one-winner/rollback behavior, independent OTP rate limits, the verification lock, Turnstile contract behavior, 24-hour/30-day session persistence, structured onboarding, canonical localized auth routes, persona-preserving workspace switches, and the fail-closed legal-source lifecycle, acquisition, untrusted normalization, MFA-bound review evidence, and immutable first-version publication contracts, including bounded oversized-block splitting. This is not staging or live-provider evidence. The clean network-install path remains unverified because actual `npm ci` was deliberately not run.
