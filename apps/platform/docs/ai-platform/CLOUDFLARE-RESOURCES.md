...
> Current checkpoint — 2026-07-31: `juro-staging` has 41 migrations through `0040`; `juro-platform-staging` version `cfb20e07-d9a9-4b55-a402-e2326c437b4a` serves 100% behind Access with five consumers (document analysis, document export, legal sources, email, retention) and one five-minute cron. Analysis exports are included in account-deletion R2 inventory and D1 evidence. Staging has no eligible completed analysis. Production Worker `juro` remains `91774ed4-72e9-47bb-b93a-a4208d490b24`. This supersedes older current-state summaries below; their historical resource IDs remain evidence of earlier checkpoints.
