...
> Current checkpoint — 2026-07-31: `juro-staging` has 41 migrations through `0040`; `juro-platform-staging` version `6cf8434d-e94c-406a-9655-02bffdf0e2d2` serves 100% behind Access with five consumers (document analysis, document export, legal sources, email, retention) and one five-minute cron. The export Queue/DLQ is configured and empty; staging has no eligible completed analysis. Production Worker `juro` remains `91774ed4-72e9-47bb-b93a-a4208d490b24`. This supersedes older current-state summaries below; their historical resource IDs remain evidence of earlier checkpoints.
