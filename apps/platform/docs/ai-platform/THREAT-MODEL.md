...
| T-20 | public status/info disclosure | local `0083` exposes only fixed component keys, bilingual operator-approved copy, public incident reference and timestamps; staff/tenant/resource identity is omitted, status hostname rejects private routes, and unavailable D1 returns bounded 503; staging/domain evidence remains absent | migrate/deploy after backup, attach only a reviewed status hostname, run negative-route and incident-rehearsal probes, keep topology/attack details and user data out of public copy |
