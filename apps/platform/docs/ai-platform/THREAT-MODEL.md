...
| T-11 | vector cross-tenant leak | empty development/staging user-document indexes exist, but contain no vectors or metadata indexes and have no bound Worker, query path, or authorization evidence; release risk remains high | physical environment isolation, pre/post D1 auth, metadata filters, leak tests |
...
| T-14 | queue replay/double charge | local v2 maps exact task kinds to identifiers-only envelopes and unique/fenced outbox/job ledgers; legacy kinds are rejected; 14 primary and 14 distinct DLQ resources exist unbound with zero producers/consumers; Wrangler declares seven producers, `consumers: []`, no malware binding, and no executable handler; no verified attachment/redrive/reconciliation or staging Worker exists | implement each side effect with provider/subject idempotency, then activate one reviewed consumer with terminal state, distinct DLQ policy, alerts, redrive, reconciliation, and staging proof |
...
| T-17 | backup/migration corruption | in-D1 copies are not backups; production/development each report 61 tables and migrations `0000`–`0004`; the verified-empty staging exception produced an exact `0000`–`0021` ledger with passing quick/FK checks, but no retrievable SQL export, checksum, protected R2 object, isolated import database, or RTO exists | portable external export, integrity check, isolated SQL restore rehearsal, rollback trigger |
