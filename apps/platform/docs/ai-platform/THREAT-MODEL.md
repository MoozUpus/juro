...
| T-14 | queue replay/double charge | local v2 uses identifiers-only envelopes and unique/fenced outbox/job ledgers; the first executable side effect is an encrypted-recipient email-change alert with provider idempotency, durable states, sequential/concurrent replay suppression, and a stale-send lease; the deployed staging Worker still has execution false and no consumers | apply migration 0030 only after backup/restore, deploy one reviewed staging email consumer with its distinct DLQ, then prove delivery, retry, redrive, reconciliation, and safe logs before enabling another handler |
