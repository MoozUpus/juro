...
| T-08 | SSRF/public URL fetch | local candidate uses `global_fetch_strictly_public`, credential-free HTTPS/443, pre-request and post-header DNS/IP checks, manual redirects, timeout, declared size/MIME/encoding limits and private-R2 integrity verification | deploy only to protected staging; add live rebinding/redirect/browser evidence; retain fail-closed malware gate |
...
| T-11 | vector cross-tenant leak | remote development/staging indexes remain empty; local `0080` uses physical environment indexes, workspace namespaces, pre-search membership, owner/latest-version D1 post-filtering, full metadata equality checks and private-R2 hash verification; focused cross-tenant and metadata-tampering tests pass, but protected staging evidence remains absent | apply `0080` only after backup, deploy exact Worker, run two-account negative search/deletion probes, inspect mutation/log evidence, keep D1 authoritative |
...
| T-15 | cost exhaustion | local `0081`–`0082` records actual usage where integrated, applies versioned daily/failure policies, blocks guarded provider calls and emits one identifiers-only operational alert per opening; staging remains through `0068`, billing reconciliation and real alert/rehearsal evidence are absent | migrate/deploy after backup, enter official prices and reviewed thresholds, prove Resend retry and manual recovery, reconcile provider billing, require every new provider call site to adopt the guard |
...
| T-17 | backup/migration corruption | in-D1 copies are not backups; production/development remain through `0004`; staging is through `0034` with 26 portable artifacts, private-R2 checksum round trips, an isolated pre-`0034` restore, and earlier disposable remote-D1 logical import drills, but no scheduled backup or operational RTO exists | repeat per-checkpoint restore drills, retention/automation, rollback trigger, production-specific protection |
...
| T-20 | public status/info disclosure | `0083` exposes only fixed component keys, bilingual operator-approved copy, public incident reference and timestamps; staff/tenant/resource identity is omitted, the attached staging-only status hostname rejects private routes, and unavailable D1 returns bounded 503. HTTPS evidence confirms `noindex` headers for `/` and `/api/status`; production remains unattached. | run protected-admin and negative-route browser checks plus an incident rehearsal; keep topology/attack details and user data out of public copy |
