...
> Current staging deployment — 2026-08-02: the D1-fenced legal-source Crawl-delay implementation is deployed only as `juro-platform-staging` version `fc2e5248-9e6f-414e-9180-bc046883f798`. D1 is through additive migrations `0058`/`0059`, with `quick_check=ok`, no foreign-key violations, and no pending migration. Production Worker, production D1, production R2, Sites, and `apps/website` remain unchanged. Exact pre-migration evidence is in `STAGING-0063-LAWYER-PROFILE-DIRECTORY-PREVIEW.md`; this deploy did not run a migration.
