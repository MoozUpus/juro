...
| Legal knowledge | legal sources and legislation updates; migrations `0025`-`0028` add source versions, sections/chunks, sync/review/publication evidence; later lifecycle/indexing slices add current activation and hybrid retrieval; local `0076` adds version-pinned user bookmarks with optional case/comment and immutable metadata-only mutation evidence | Advice scenario/version/link records, internal materials, complete bulk discovery, wider real-markup validation, and remote bookmark/browser evidence |
