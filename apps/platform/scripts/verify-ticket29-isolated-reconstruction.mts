...
const EVIDENCE_PREFIX = "legal-corpus/complete-v1/";
...
function publisherTokensMatch(input: {
...
async function main(): Promise<void> {
  ...
  const identityRows = database.prepare(`SELECT source_id AS sourceId,source_document_id AS documentId,
      source_revision_sha256 AS sourceRevisionSha256,language,script,textual_authority AS textualAuthority,
      valid_from AS validFrom,valid_to AS validTo,instrument_id AS instrumentId,
      official_expression_id AS officialExpressionId,text_revision_id AS textRevisionId,
      provision_concept_id AS provisionConceptId,provision_rendition_id AS provisionRenditionId,
      ...
      legacy_target_publisher_revision_token AS legacyTargetPublisherRevisionToken,
      source_publisher_revision_token AS sourcePublisherRevisionToken,
      publisher_provision_token AS publisherProvisionToken,applicability_identity AS applicabilityIdentity
    FROM legal_complete_corpus_records WHERE run_id=?`).iterate(RUN_ID) as Iterable<Record<string, string | null>>;
  for (const row of identityRows) {
    const instrumentId = `instrument:${sha256(row.documentId!)}`;
    const officialExpressionId = `expression:${sha256(
      `${instrumentId}\u0000${row.language}\u0000${row.script}\u0000${row.textualAuthority}`)}`;
    const textRevisionId = `revision:${sha256([row.documentId, row.language, row.script,
    ...
    const provisionRenditionId = `rendition:${sha256([row.documentId, provisionConceptId,
    ...
    const legacyProvisionConceptId = `concept:${sha256(`${row.documentId}|${row.publisherProvisionToken}`)}`;
    const legacyCurrentRenditionId = `rendition:${sha256(`${legacyProvisionConceptId}\u0000${legacyRevisionId}`)}`;
  ...
  const locatorMismatches = (database.prepare(`SELECT
      (SELECT count(*) FROM legal_complete_corpus_records r
        ...
        LEFT JOIN legal_complete_corpus_objects provision ON provision.run_id=r.run_id
          ...
          AND ((provision.r2_key LIKE 'legal-corpus/complete-v1/%'
