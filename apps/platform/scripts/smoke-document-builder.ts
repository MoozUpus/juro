...
async function main(): Promise<void> {
  ...
  const ownerBootstrap = await api<{ storage: { d1: boolean; r2: boolean }; user: { id: string } }>("/api/document-builder/bootstrap", { user: ownerEmail });
  ...
  const created = await api<{ document: { id: string; revision: number; status: string } }>("/api/document-builder/drafts", {
    method: "POST",
    user: ownerEmail,
  ...
  await api(`/api/document-builder/documents/${documentId}`, { user: collaboratorEmail, expected: 404 });

  const saved = await api<{ revision: number; status: string }>(`/api/document-builder/documents/${documentId}`, {
    method: "PUT",
    user: ownerEmail,
  ...
  const generated = await api<{ status: string; files: Record<"docx" | "pdf" | "zip", { id: string; url: string; mimeType: string; size: number }> }>(`/api/document-builder/documents/${documentId}/generate`, {
    method: "POST",
    user: ownerEmail,
    json: {},
  });
  ...
  const uzCreated = await api<{ document: { id: string } }>("/api/document-builder/drafts", {
    method: "POST",
    user: ownerEmail,
  ...
  const uzGenerated = await api<{ files: Record<"docx" | "pdf", { url: string }> }>(`/api/document-builder/documents/${uzCreated.data.document.id}/generate`, {
    method: "POST",
    user: ownerEmail,
    json: {},
  });
  ...
  await api(`/api/document-builder/documents/${uzCreated.data.document.id}`, { method: "DELETE", user: ownerEmail });

  const listed = await api<{ documents: Array<{ id: string }>; total: number }>("/api/document-builder/documents?search=Интеграционная", { user: ownerEmail });
  assert.ok(listed.data.documents.some((item) => item.id === documentId));
  assert.ok(listed.data.total >= 1);

  const duplicate = await api<{ document: { id: string } }>("/api/document-builder/documents", {
    method: "POST",
    user: ownerEmail,
  ...
  const contact = await api<{ contact: { id: string } }>("/api/document-builder/contacts", { method: "POST", user: ownerEmail, json: contactInput, expected: 201 });
  const contacts = await api<{ contacts: Array<{ id: string }> }>("/api/document-builder/contacts", { user: ownerEmail });
  assert.ok(contacts.data.contacts.some((item) => item.id === contact.data.contact.id));
  await api(`/api/document-builder/contacts/${contact.data.contact.id}`, { method: "PUT", user: ownerEmail, json: { ...contactInput, label: "Обновлённый контакт" } });
  await api(`/api/document-builder/contacts/${contact.data.contact.id}`, { method: "DELETE", user: ownerEmail });

  const publicShare = await api<{ url: string }>(`/api/document-builder/documents/${documentId}/share`, {
    method: "POST",
    user: ownerEmail,
    json: { action: "create" },
  });
  ...
  await api(`/api/document-builder/documents/${documentId}/collaboration`, {
    method: "POST",
  ...
  const proposal = await api<{ proposalId: string }>(`/api/document-builder/documents/${documentId}/collaboration`, {
    method: "POST",
    user: collaboratorEmail,
    json: { action: "proposal", oldText, newText: "Настоящая расписка добровольно составлена между:" },
    expected: 201,
  });
  const accepted = await api<{ applied: boolean }>(`/api/document-builder/documents/${documentId}/collaboration`, {
    method: "POST",
    user: ownerEmail,
    json: { action: "accept_proposal", proposalId: proposal.data.proposalId },
  });
  assert.equal(accepted.data.applied, true);
  await api(`/api/document-builder/documents/${documentId}/collaboration`, { method: "POST", user: collaboratorEmail, json: { action: "confirm_data" } });

  await api(`/api/document-builder/documents/${documentId}`, { method: "PATCH", user: ownerEmail, json: { action: "confirm_agreement" } });
  const agreedDocument = await api<{ document: { revision: number; status: string; autoContent: string; finalContent: string } }>(`/api/document-builder/documents/${documentId}`, { user: ownerEmail });
  ...
  const afterAgreedEdit = await api<{ status: string }>(`/api/document-builder/documents/${documentId}`, {
    method: "PUT",
    user: ownerEmail,
  ...
  const signed = await api<{ file: { id: string }; status: string }>(`/api/document-builder/documents/${documentId}/signed-file`, {
    method: "POST",
    user: ownerEmail,
    body: signedForm,
    expected: 201,
  ...
  const collaboratorSigned = await download(`/api/document-builder/documents/${documentId}/files/${signed.data.file.id}?inline=1`, collaboratorEmail);
  assert.equal(new TextDecoder().decode(collaboratorSigned.bytes.slice(0, 5)), "%PDF-");

  const consultation = await api<{ request: { contextAttached: boolean }; handoffUrl: string }>("/api/document-builder/consultations", {
    method: "POST",
    user: ownerEmail,
  ...
  const ownerNotifications = await api<{ notifications: unknown[] }>("/api/document-builder/notifications", { user: ownerEmail });
  assert.ok(ownerNotifications.data.notifications.length > 0);

  await api(`/api/document-builder/documents/${documentId}`, { method: "PATCH", user: ownerEmail, json: { action: "archive" } });
  await api(`/api/document-builder/documents/${documentId}`, { method: "PATCH", user: ownerEmail, json: { action: "restore" } });

  const removed = await api<{ preservedSignedFileId: string }>(`/api/document-builder/documents/${documentId}?signed=keep`, { method: "DELETE", user: ownerEmail });
  assert.equal(removed.data.preservedSignedFileId, signed.data.file.id);

  const standaloneList = await api<{ standaloneFiles: Array<{ id: string }> }>("/api/document-builder/documents", { user: ownerEmail });
  assert.ok(standaloneList.data.standaloneFiles.some((item) => item.id === signed.data.file.id));
  const standaloneShare = await api<{ share: { url: string; code: string } }>(`/api/document-builder/standalone-files/${signed.data.file.id}/share`, {
    method: "POST",
    user: ownerEmail,
    json: { action: "create" },
    expected: 201,
  ...
  assert.ok(token);
  await api(`/api/document-builder/standalone-signed-shares/${token}/verify`, { method: "POST", json: { code: "9999" }, expected: 403 });
  const verified = await api<{ viewerUrl: string }>(`/api/document-builder/standalone-signed-shares/${token}/verify`, {
    method: "POST",
  ...
  const replacedShare = await api<{ share: { url: string; code: string } }>(`/api/document-builder/standalone-files/${signed.data.file.id}/share`, {
    method: "POST",
    user: ownerEmail,
    json: { action: "create" },
    expected: 201,
  ...
  await api(`/api/document-builder/standalone-signed-shares/${token}/verify`, { method: "POST", json: { code: standaloneShare.data.share.code }, expected: 403 });

  await api(`/api/document-builder/documents/${duplicate.data.document.id}`, { method: "DELETE", user: ownerEmail });
  await api(`/api/document-builder/standalone-files/${signed.data.file.id}`, { method: "DELETE", user: ownerEmail });
