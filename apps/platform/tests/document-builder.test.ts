...
test("код доступа состоит из четырех цифр, хешируется и шифруется", async () => { const route = await readFile(new URL("app/api/document-builder/standalone-files/[id]/share/route.ts", root), "utf8"); const cryptoSource = await readFile(new URL("lib/document-builder/share-links/crypto.ts", root), "utf8"); assert.match(cryptoSource, /padStart\(4/); assert.match(route, /access_code_hash/); assert.match(route, /access_code_ciphertext/); assert.match(route, /protectSignedShareSecret/); });
...
test("print mode скрывает форму и оставляет документ", async () => { const css = await readFile(new URL("app/_document-builder/document-builder.css", root), "utf8"); assert.match(css, /@media print/); assert.match(css, /dbt-print-only/); assert.match(css, /visibility:\s*hidden/); });
