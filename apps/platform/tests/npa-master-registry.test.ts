...
test("NPA registry is exactly the mandatory 100 plus a separate future successor", () => {
  ...
  assert.equal(NPA_MASTER_TARGETS.find((target) => target.documentKey === "rights_of_persons_with_disabilities")?.verifiedSourceSeed,
    "https://lex.uz/ru/docs/5049549");
...
test("a corrected curated LexUZ source is prioritized without re-enabling a rejected route", async () => {
  ...
  try {
    ...
    const expectedJobId = await officialLexCorpusFetchJobId({
      ...
      idempotencyScope: "npa-current-card:v9:civil_code_part_1:2026-09-11",
    });
...
test("unresolved NPA identity reviews take priority over routine verified-card refreshes", async () => {
  ...
  try {
    ...
    await enqueueOfficialLexCorpusDocument(env, {
      ...
      idempotencyScope: "npa-current-card:v9:customs_code:2026-09-11",
    });
    await enqueueOfficialLexCorpusDocument(env, {
      sourceUrl: "https://lex.uz/ru/docs/7283074", now,
      idempotencyScope: "npa-current-card:v9:telecommunications:2026-09-11",
    });
