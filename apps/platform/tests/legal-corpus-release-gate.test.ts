...
import {
  LEGAL_CORPUS_TEST_NOW as now,
  validLegalCorpusReleaseEvidence as validEvidence,
...
test("release capacity evidence may name the active shard-2 staging database", () => {
  const evidence = validEvidence();
  evidence.d1Capacity.databaseName = "juro-staging-corpus-shard-2";
  assert.doesNotThrow(() => legalCorpusReleaseEvidenceSchema.parse(evidence));
});
...
test("release capacity evidence may name the prepared shard-4 corpus database", () => {
  const evidence = validEvidence();
  evidence.d1Capacity.databaseName = "juro-staging-corpus-shard-4";
  assert.doesNotThrow(() => legalCorpusReleaseEvidenceSchema.parse(evidence));
});
