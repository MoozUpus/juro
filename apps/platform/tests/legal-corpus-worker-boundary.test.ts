...
test("version debt preserves current-corpus fetch capacity within the request budget", () => {
  assert.deepEqual(legalCorpusVersionSlotIndexes({ ingestionBudget: 9, queuedVersionJobs: 499 }), [3]);
  assert.deepEqual(legalCorpusVersionSlotIndexes({ ingestionBudget: 9, queuedVersionJobs: 500 }), [3, 4, 5, 6, 7, 8]);
  assert.deepEqual(legalCorpusVersionSlotIndexes({ ingestionBudget: 10, queuedVersionJobs: 500 }), [3, 4, 5, 6, 7, 8, 9]);
...
test("main application scheduler cannot import or invoke heavy corpus work", () => {
  ...
  assert.match(
    corpusWorker,
    /const compactedSparseJsonChunks = sparseCompressionMaintenanceEnabled/u,
  );
...
test("dedicated Worker is route-free, production-fail-closed and staging-bounded", () => {
  ...
  assert.equal(config.env.staging.vars.LEGAL_CORPUS_LIVE_LEXUZ_ENABLED, "false");
  ...
  assert.equal(config.env.staging.vars.LEGAL_CORPUS_SHADOW_MODE, "false");
