...
const migrationByTag = (tag: string) => journal.entries.find((entry) => entry.tag === tag);
const englishPolicyEvidenceEntry = migrationByTag("0151_english_policy_evidence");
const passwordChangedEmailRetryEntry = migrationByTag("0152_password_changed_email_retry");
const pendingRegistrationRetentionEntry = migrationByTag("0153_pending_registration_retention");
const englishLocaleFoundationEntry = migrationByTag("0154_english_locale_foundation");
const legacyOfficialCorpusRetirementEntry = migrationByTag("0150_retire_legacy_official_corpus_storage");
const legacyOfficialCorpusSchemaRemovalEntry = migrationByTag("0151_remove_legacy_official_corpus_schemas");
...
assert.ok(
  caseLifecycleEvidenceEntry,
  "Drizzle journal must contain migration 0093",
);
assert.ok(
