...
function seed(sqlite: ReturnType<typeof sqliteD1Fixture>["sqlite"]) {
  ...
  sqlite.prepare("INSERT INTO lawyer_profiles(id,user_id,display_name,status,public_approved_at,created_at,updated_at) VALUES ('lawyer-profile','lawyer','Юрист JURO','public_approved',?,?,?)").run(now, now, now);
