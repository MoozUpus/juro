...
test("admin handoff route requires same-origin write protection and current MFA", async () => {
  ...
  assert.match(adminWorker, /<html lang="ru" data-theme="system">/u);
  assert.match(adminWorker, /new Set\(\["system","light","dark"\]\)/u);
  assert.match(adminWorker, /modes\.has\(cookieMode\)\?cookieMode:modes\.has\(storedMode\)\?storedMode:"system"/u);
  assert.match(adminWorker, /localStorage\.setItem\("juro_theme",next\)/u);
  assert.match(adminWorker, /Domain=\.juro\.uz/u);
  assert.match(adminWorker, /data-admin-theme="system"/u);
  assert.match(adminWorker, /data-admin-theme="light"/u);
  assert.match(adminWorker, /data-admin-theme="dark"/u);
  assert.match(adminWorker, /html\[data-theme=dark\]\{color-scheme:dark/u);
  assert.match(adminWorker, /@media\(prefers-color-scheme:dark\)\{html\[data-theme=system\]/u);
  assert.match(adminWorker, /securityHeaders\(scriptNonce\)/u);
  assert.match(adminWorker, /script-src \$\{scriptSource\}/u);
  assert.doesNotMatch(adminWorker, /script-src 'unsafe-inline'/u);
