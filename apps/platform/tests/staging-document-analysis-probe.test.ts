...
test("document analysis probe has one durable, versioned claim per UTC window", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      "staging-document-analysis-v2-20260812",
    );
    assert.equal(
      stagingDocumentAnalysisProbeExecutionKey(secondWindow),
      "staging-document-analysis-v2-20260813",
    );
    ...
    assert.deepEqual(first, {
      claimed: true,
      probeExecutionKey: "staging-document-analysis-v2-20260812",
      ...
      [
        {
          ...
          idempotencyKey: "staging-document-analysis-probe:staging-document-analysis-v2-20260812",
          status: "running",
          errorCode: null,
        },
        {
          ...
          idempotencyKey: "staging-document-analysis-probe:staging-document-analysis-v2-20260813",
          status: "running",
          errorCode: null,
        },
      ],
...
test("failed one-shot document probe remains blocked until an operator resets its durable claim", async () => {
  ...
  try {
    ...
    const idempotencyKey = "staging-document-analysis-probe:staging-document-analysis-v2-20260812";
    sqlite.prepare(
